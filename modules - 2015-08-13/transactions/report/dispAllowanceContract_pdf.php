...
class PDF extends FPDF
{
	...
	function Content($arrContract) {
		...
		$Draft = ($arrContract['stsNo']!="")? "DAC ".$arrContract['stsNo']:"Draft Only";
		...
		if($arrContract['nbrApplication']=='1'){
			...
			$this->Cell(100,5,date('M d, Y',strtotime($arrContract['impStartDate'])). " to " .date('M d, Y',strtotime($arrContract['impEndDate'])),0,1,'L');
		}else{
			$this->SetFont('Arial', '', '11');
            //$this->Cell(100,5,date('M d, Y',strtotime($arrContract['applyDate'])). " to " .date('M d, Y',strtotime($arrContract['endDate'])),0,1,'L');
			$this->Cell(100,5,date('M d, Y',strtotime($arrContract['impStartDate'])). " to " .date('M d, Y',strtotime($arrContract['impEndDate'])),0,1,'L');
		...
		$this->Ln(1);
		$this->SetFont('Arial', 'B', '12');
		$this->Cell(65,5,"Monthly Display Fee per Unit: ",0,0,'L');	
		$this->SetFont('Arial', '', '11');
		...
		if($arrContract['stsPaymentMode']=='C'){
			$this->Ln(1);
			$this->SetFont('Arial', 'B', '12');
			$this->Cell(65,5,"Amount: ",0,0,'L');	
			$this->SetFont('Arial', '', '11');
			$this->Cell(100,5,"Php ".number_format($arrContract['stsAmt'],2),0,1,'L');
			
			$this->Ln(1);
			$this->SetFont('Arial', 'B', '12');
			$this->Cell(65,5,"12% VAT: ",0,0,'L');	
			$this->SetFont('Arial', '', '11');
			$this->Cell(100,5,"Php ".number_format($arrContract['stsVatAmt'],2),0,1,'L');
		}
		$this->Ln(1);
		$this->SetFont('Arial', 'B', '12');
		$this->Cell(65,5,"Total Display Fee per Month: ",0,0,'L');	
		$this->Cell(100,5,"Php ".number_format($arrContract['stsAmt']+$arrContract['stsVatAmt'],2),0,1,'L');
		...
		$this->SetFont('Arial', '', '11');
		$this->Ln(1);
		$this->MultiCell(200,6,"1. Above amount is inclusive of all applicable government taxes.");
		$this->Ln(1);
		$this->MultiCell(200,6,"2. The company agress to maintain cleanliness, orderliness, attractive and appealing visual standards in the display area.");
		$this->Ln(1);
		$this->MultiCell(200,6,"3. Only items currently carried by Puregold in its product mix should be included in the display. New items to feature and/or for inclusion should have the prior approval of Merchandising Department and coordination with the Store Manager.");
		$this->Ln(1);
		$this->MultiCell(200,6,"4. Product signages must be in consonance and harmony with the Puregold product tags and signages. Other merchandising and advertising collaterals should have the prior approval of Merchandising and coordination with the Store Manager");
	  	$this->Ln(1);
	  	$this->MultiCell(200,6,"5. This contract may be terminated at any time by either of the parties, provided that a written notice is served thirty (30) days before the requested termination date.");
		$this->Ln(1);
		$this->MultiCell(200,6,"6. This contract is renewable at the end of the contract period in favor of the company. However, Puregold reserves the right to grant the display area to the other interested parties, if the company fails to confirm the new agreement thirty (30) days before the current contract period.");
		$this->Ln(2);
