...
if ($_SESSION['sts-username'] == "") {
	...
	switch($_POST['act']) {
		case 'saveRentables':
			$result = $daObj->addStsRentableDtlperStr($_POST);
			if($result!=0) {
...
if($_GET['action']=='getRentables'){
	//$arr = $daObj->getRentableDtls($_GET['refNo'],$_GET['displaySpecs'],$_GET['strCode']);
	$arr = $daObj->getRentableDtlsTactical($_GET['refNo'],$_GET['displaySpecs'],$_GET['strCode'],$_GET['startDate'],$_GET['endDate']); 
?>
<h2 class="ui-widget-content">Rentable List</h2>
<form name="frmRentable2" id="frmRentable2">
