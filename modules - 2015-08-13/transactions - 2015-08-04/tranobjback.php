...
class transObj extends commonObj {
	...
	function countRegSTS(){
		$sql = "Select count(stsRefNo) as count From tblStsHdr";	
		return $this->getSqlAssoc($this->execQry($sql));
	}
	function getPaginatedRegSTS($sidx,$sord,$start,$limit){	
	...
	function getPaginatedRegSTSComp($sidx,$sord,$start,$limit,$compCode){	
		if($compCode=='0'){
			$comp = "";	
		}else{
		...
		$sql = "SELECT
			...
			LEFT OUTER JOIN tblsuppliers ON tblstshdr.suppCode = tblsuppliers.suppCode
			LEFT OUTER JOIN tblcompany ON tblstshdr.stsComp = tblcompany.compCode
			WHERE stsType = '1' $comp
			ORDER BY $sidx $sord LIMIT $start , $limit
			";
		return $this->getArrRes($this->execQry($sql));
	}
	function searchRegSTS($sidx,$sord,$start,$limit,$searchField,$searchString){
		$sql = "SELECT
			tblstshdr.stsRefNo,
			tblstshdr.suppCode,
			tblstshdr.stsDept,
			...
			LEFT OUTER JOIN tblsuppliers ON tblstshdr.suppCode = tblsuppliers.suppCode
			LEFT OUTER JOIN tblcompany ON tblstshdr.stsComp = tblcompany.compCode
			WHERE $searchField = '$searchString' AND stsType = '1' ORDER BY $sidx $sord LIMIT $start , $limit
			";
	...
	function findSupplier($terms){
		$sql = "SELECT suppCode, suppName FROM tblSuppliers WHERE (suppCode like '%$terms%') or (suppName like '%$terms%') 	
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsAmt, stsRemarks, 
			...
			'{$arr['txtNoApplications']}', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-minCode']."', '{$arr['cmbCompCode']}', 'O', '1')";
	...
	function updateHeader($arr){
		$sqlUpdate = "UPDATE tblStsHdr SET 
			...
			WHERE stsRefNo = '{$arr['refNo']}' AND stsComp = '$compCode'";
		$trans = $this->beginTran();
		if ($trans) {
	...
	function releaseSTS($refNo,$compCode){
		...
		$sqlUpdateHeader = "UPDATE tblstshdr SET stsStartNo = '$startingSTS', stsEndNo = '$tempSTSNo', approvedBy = '".$_SESSION['sts-userId']."', dateApproved = '".date('Y-m-d')."', stsTag = 'Y', stsDate = '".date('Y-m-d')."', stsStat = 'R'
