<?php
	...
	for ($a=0;$a<$t_ci_header;$a++){ 
		...
		if(isset($_POST["check$a"])) {
			...
			$query_ci_header="SELECT * FROM tblMakeupHeader WHERE (adjNumber = $check_ci_no)";
			...
			if (trim($adj_type)=="MK") {
				$adj_desc = "Product Makup Adjustments Report";
			} else  {
				$adj_desc = "Product Breakup Adjustments Report";
			}
			...
			################################################# 
			$query_ci_detail="SELECT * FROM tblMakeupDtl WHERE (adjNumber = $check_ci_no) ORDER BY adjNumber, prdMkTag ASC";
			...
			for ($b=0;$b<$m_page;$b++){
				...
				$pdf->Cell($m_width_3_fields,5,"Program-Id : MK02P",0,0);
				$pdf->Cell($m_width_3_fields,5,$adj_desc,0,0,'C');
				$pdf->Cell($m_width_3_fields,5,$page,0,1,'R');
				$pdf->Cell($m_width_3_fields,5,'',0,0);
				$pdf->Cell($m_width_3_fields,5,$date,0,0,'C');
				$pdf->Cell($m_width_3_fields,5,'',0,1,'R');
				$pdf->Cell($m_width_3_fields,5,$from_loc,0,1);
				$pdf->ln();
				$pdf->Cell($m_width, 0, '', 1, 0);
				$pdf->ln();
				$pdf->Cell(20,$dtl_ht, 'From/To', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'SKU', 0, 0);
				$pdf->Cell(80,$dtl_ht, 'Description', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'UM', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'Quantity', 0, 0,'R');
				...
				if ($tmp_rec <= 0) { /// 1 page consume
					...
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									...
									$grid_tag=mssql_result($result_ci_detail,$i,"prdMkTag");
									$grid_um=mssql_result($result_ci_detail,$i,"umCode");
									if (trim($adj_type)=="MK") {
										if ($grid_tag==1) {
											$grid_sku=mssql_result($result_ci_detail,$i,"prdParent");
											$grid_from_to= "To ";
										} else {
											$grid_sku=mssql_result($result_ci_detail,$i,"prdChild");
											$grid_from_to= "   From ";
										}
									} else {
										if ($grid_tag==1) {
									...
									$grid_adj_cost = number_format($grid_adj_cost, 2);                                
									$grid_ext_cost = number_format($grid_ext_cost, 2);    
									$pdf->Cell(20,$dtl_ht, $grid_from_to, 0, 0);          
									$pdf->Cell(20,$dtl_ht, $grid_sku, 0, 0);
									$pdf->Cell(80,$dtl_ht, $grid_desc, 0, 0);
									$pdf->Cell(20,$dtl_ht, $grid_um, 0, 0);
				...
				if ($tmp_rec <= 0) { /// 1 page consume
					...
					$pdf->ln();
					$x_cost_total_grand = number_format($x_cost_total_grand, 2);        #
					$pdf->Cell(35,$dtl_ht, '', 0, 0);
