<?
	...
	include("price_release_function.php");
	...
	if($_GET['action'] == 'doRelease'){
		foreach ((array)($_POST['chkbox']) as $index => $value){
			PriceRelease($value,$compCode,$user);
			echo "<script>location.href='{$_SERVER['PHP_SELF']}'</script>";
		}	
	}
		
	$qryprEvntHdr = "SELECT * FROM tblPriceEventHeader WHERE compCode = '{$compCode}' AND prEventStatus = 'O' ";
	if($_GET['action'] == 'DoSearch'){
		if(!empty($_POST['txtSearch'])){
			if($_POST[cmbSearch] == 'EVENT NUMBER'){	
				$qryprEvntHdr .= " AND CSTEVENTNUMBER LIKE '{$_POST['txtSearch']}%' ";
			}else{
				$qryprEvntHdr .= " AND CSTCOSTDESC LIKE '{$_POST['txtSearch']}%' ";
			}
		}
	}
		...
		<form name="frmprEvntEntry" id="frmprEvntEntry" action="<?=$_SERVER['PHP_SELF']?>" method="POST">
			...
			<table align="center" border="0">
				<tr nowrap="wrap" align="left" bgcolor="#6AB5FF">
					...
					<?php echo "<< PRICE EVENT RELEASE >>";?>        
			        </th>
				</tr>
				<tr nowrap="wrap" align="left" bgcolor="#6AB5FF">
					<th height="23" colspan="6" nowrap="nowrap" class="style6" align="center">
					    <a href="#" onClick="ViewReleasePrice();">View All Release Price Events Report</a>					
			        </th>
				</tr>
				<tr bgcolor="#DEEDD1" >
					...
					<td class="style3" align="center" width="400">
...
<script>
	...
	{
		var nwidth = screen.availWidth - 800;
		...
		mywindow = window.open("price_viewrelease_price.php","PriceEvents",winattbr);	
