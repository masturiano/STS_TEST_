<?php
	...
	$query_event_date = mssql_query("SELECT * FROM tblConsessHdr WHERE prEventNumber = '$E_No'");
	...
	if ($num_loc>0) {
		$Desc=mssql_result($result_loc1,0,8);
		$Start=mssql_result($result_loc1,0,5);
		$End=mssql_result($result_loc1,0,6);
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Price Event List (Concessionaire)",0,0,'C');
