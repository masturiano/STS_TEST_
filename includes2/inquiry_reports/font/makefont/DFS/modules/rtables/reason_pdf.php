<?php
	...
	switch ($search_selection) {
		case "all_record":
			$title = "Reason for Adjustments Listing";
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : REASONL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				...
				$pdf->Cell(100,$dtl_ht, 'Description', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'GL Major', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'GL Minor', 0, 0);
				break;
		}
		$pdf->ln();
		$pdf->Cell($m_width, 0, '', 1, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_trans,$i,"rsnCode");
											$grid_name=mssql_result($result_trans,$i,"rsnDesc");
											$grid_major=mssql_result($result_trans,$i,"rsnGlMajor");
											$grid_minor=mssql_result($result_trans,$i,"rsnGlMinor");
											$pdf->Cell(20,$dtl_ht, $grid_code, 0, 0);
											$pdf->Cell(100,$dtl_ht, $grid_name, 0, 0);
											$pdf->Cell(20,$dtl_ht, $grid_major, 0, 0);
											$pdf->Cell(20,$dtl_ht, $grid_minor, 0, 0);
											$pdf->ln();
											break;
									}
						$i++;
