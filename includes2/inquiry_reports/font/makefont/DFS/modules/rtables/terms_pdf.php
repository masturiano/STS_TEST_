<?php
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : TERMSL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				$pdf->Cell(20,$dtl_ht, 'Terms', 0, 0);
				$pdf->ln();
				$pdf->Cell(20,$dtl_ht, 'Code', 0, 0);
				$pdf->Cell(100,$dtl_ht, '   Terms Description', 0, 0);
				break;
		}
		$pdf->ln();
		$pdf->Cell($m_width, 0, '', 1, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_trans,$i,"trmCode");
											$grid_name=mssql_result($result_trans,$i,"trmDesc");
