<?php
	...
	switch ($search_selection) {
		case "all_record":
			$title = "Price Type Listing";
			$m_line = 70;  ///maximum line
			break;
	}
	####################company name##################################
	$result_pt=mssql_query($search_query);
	$num_pt = mssql_num_rows($result_pt);
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		if ($tmp_rec <= 0) { /// 1 page consume
			...
			$entries="Entries ".$tmp_first." to ".$tmp_last." of ".$num_pt." record/s";
			$page="Page ".$j." of ".$m_page;
		} else {
			$tmp_last_more=$j*$m_line;
			$entries="Entries ".$tmp_first." to ".$tmp_last_more." of ".$num_pt." record/s";
			$page="Page ".$j." of ".$m_page;
		}
		$pdf->Cell($m_width_3_fields,5,$newdate,0,0);
		$pdf->Cell($m_width_3_fields,5,$comp_name,0,0,'C');
		$pdf->Cell($m_width_3_fields,5,$page,0,1,'R');
		$pdf->Cell($m_width_3_fields,5,"Report ID : PriceTypeL",0,0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_pt,$i,"prTypeCode");
											$grid_desc=mssql_result($result_pt,$i,"prTypeDesc");
											$grid_pre=mssql_result($result_pt,$i,"prPrecedence");
											$grid_acct1=mssql_result($result_pt,$i,"prAcct1");
											$grid_acct2=mssql_result($result_pt,$i,"prAcct2");
