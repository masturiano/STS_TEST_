<?php
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : CUSTL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				...
				$pdf->Cell(50,$dtl_ht, 'Contact No', 0, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_loc,$i,"custCode");
											$grid_name=mssql_result($result_loc,$i,"custName");
											$grid_addr1=mssql_result($result_loc,$i,"custAddr1");
											$grid_tel=mssql_result($result_loc,$i,"custTel");
											$grid_fax=mssql_result($result_loc,$i,"custFax");
											$grid_type=mssql_result($result_loc,$i,"custType");
											$pdf->Cell(20,$dtl_ht, $grid_code, 0, 0);
											$pdf->Cell(60,$dtl_ht, $grid_name, 0, 0);
											$pdf->Cell(60,$dtl_ht, $grid_addr1, 0, 0);
											$pdf->Cell(50,$dtl_ht, $grid_tel, 0, 0);
											$pdf->Cell(20,$dtl_ht, $grid_type, 0, 0);
											$pdf->ln();
											break;
									}
						$i++;
