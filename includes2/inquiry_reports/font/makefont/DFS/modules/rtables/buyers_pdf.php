<?php
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : BUYERSL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				...
				$pdf->Cell(58,$dtl_ht, 'Name', 0, 0);
				$pdf->Cell(58,$dtl_ht, 'Address', 0, 0);
				$pdf->Cell(25,$dtl_ht, 'Contact No', 0, 0);
				break;
		}
		$pdf->ln();
		$pdf->Cell($m_width, 0, '', 1, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_trans,$i,"buyerCode");
											$grid_name=mssql_result($result_trans,$i,"buyerName");
											$grid_address=mssql_result($result_trans,$i,"buyerAddr1");
											$grid_contact=mssql_result($result_trans,$i,"buyerTelNo");
											$pdf->Cell(20,$dtl_ht, $grid_code, 0, 0);
											$pdf->Cell(58,$dtl_ht, $grid_name, 0, 0);
											$pdf->Cell(58,$dtl_ht, $grid_address, 0, 0);
											$pdf->Cell(25,$dtl_ht, $grid_contact, 0, 0);
											$pdf->ln();
											break;
									}
						$i++;
