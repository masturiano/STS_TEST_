<?php
	...
	switch ($search_selection) {
		case "all_record":
			$title = "Transaction Types Listing";
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : TRANSTYPEL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				...
				$pdf->Cell(20,$dtl_ht, 'Tag', 0, 0);
				$pdf->Cell(20,$dtl_ht, 'Operation', 0, 0);
				break;
		}
		$pdf->ln();
		$pdf->Cell($m_width, 0, '', 1, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_trans,$i,"trnTypeCode");
											$grid_name=mssql_result($result_trans,$i,"trnTypeDesc");
											$grid_tag=mssql_result($result_trans,$i,"trnInvTag");
											$grid_oprn=mssql_result($result_trans,$i,"trnInvOprn");
											$pdf->Cell(20,$dtl_ht, $grid_code, 0, 0);
											$pdf->Cell(100,$dtl_ht, $grid_name, 0, 0);
											$pdf->Cell(20,$dtl_ht, $grid_tag, 0, 0);
											$pdf->Cell(20,$dtl_ht, $grid_oprn, 0, 0);
											$pdf->ln();
											break;
									}
						$i++;
