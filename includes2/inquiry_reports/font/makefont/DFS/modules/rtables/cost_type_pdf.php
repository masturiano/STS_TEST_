<?php
	...
	switch ($search_selection) {
		case "all_record":
			$title = "Cost Type Listing";
			$m_line = 70;  ///maximum line
			break;
	}
	####################company name##################################
	$result_ct=mssql_query($search_query);
	$num_ct = mssql_num_rows($result_ct);
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		if ($tmp_rec <= 0) { /// 1 page consume
			...
			$entries="Entries ".$tmp_first." to ".$tmp_last." of ".$num_ct." record/s";
			$page="Page ".$j." of ".$m_page;
		} else {
			$tmp_last_more=$j*$m_line;
			$entries="Entries ".$tmp_first." to ".$tmp_last_more." of ".$num_ct." record/s";
			$page="Page ".$j." of ".$m_page;
		}
		$pdf->Cell($m_width_3_fields,5,$newdate,0,0);
		$pdf->Cell($m_width_3_fields,5,$comp_name,0,0,'C');
		$pdf->Cell($m_width_3_fields,5,$page,0,1,'R');
		$pdf->Cell($m_width_3_fields,5,"Report ID : CostTypeL",0,0);
		...
		switch ($search_selection) {
			case "all_record":
				...
				$pdf->Cell(60,$dtl_ht, 'Description', 0, 0);
				$pdf->Cell(30,$dtl_ht, 'Precedence', 0, 0);
				$pdf->Cell(30,$dtl_ht, 'Major GL', 0, 0);
				$pdf->Cell(30,$dtl_ht, 'Minor GL', 0, 0);
				$pdf->Cell(30,$dtl_ht, 'Tag', 0, 0);
				break;
		}
		$pdf->ln();
		$pdf->Cell($m_width, 0, '', 1, 0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_ct,$i,"cstTypeCode");
											$grid_desc=mssql_result($result_ct,$i,"cstTypeDesc");
											$grid_pre=mssql_result($result_ct,$i,"cstPrecedence");
											$grid_acct1=mssql_result($result_ct,$i,"cstAcct1");
											$grid_acct2=mssql_result($result_ct,$i,"cstAcct2");
											$grid_tag=mssql_result($result_ct,$i,"cstTag");
											$pdf->Cell(20,$dtl_ht, $grid_code, 0, 0);
											$pdf->Cell(60,$dtl_ht, $grid_desc, 0, 0);
											$pdf->Cell(30,$dtl_ht, $grid_pre, 0, 0);
											$pdf->Cell(30,$dtl_ht, $grid_acct1, 0, 0);
											$pdf->Cell(30,$dtl_ht, $grid_acct2, 0, 0);
											$pdf->Cell(30,$dtl_ht, $grid_tag, 0, 0);
											$pdf->ln();
											break;
									}
						$i++;
