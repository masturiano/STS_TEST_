<?php
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : ALLWTYPESL",0,0);
				...
				if ($total_line <=$max_tot_line) {
					for ($i=$tmp_first;$i <= $tmp_last;$i++){
						$i--;							
									switch ($search_selection) {
										case "all_record":
											$grid_code=mssql_result($result_trans,$i,"allwTypeCode");
											$grid_name=mssql_result($result_trans,$i,"allwCostTag");
											$grid_tag=mssql_result($result_trans,$i,"allwCostTag");
