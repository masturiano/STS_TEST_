<?php
	...
	$db = new DB;
	$db->connect();
	$db->query("SELECT tblPoHeader1.poNumber, convert(varchar,tblPoHeader1.poDate,101) as poDate, convert(varchar,tblPoHeader1.poReopenDate, 101) as poReopenDate,
                (tblSuppliers.suppName + tblSuppliers.suppName) as suppName, dbo.tblPoHeader1.poTotExt
				FROM tblPoHeader1 INNER JOIN
                tblSuppliers ON tblPoHeader1.suppCode = tblSuppliers.suppCode");
...
<form name="frmRCR" method="post">
	...
	foreach($procquery as $ctr)
	{
      ...
      <tr bgcolor="<?php echo $bgcolor; ?>" <?php echo $on_mouse; ?>>
        ...
        <td width="5%" align="center"><a  onclick="javascript:navigator.plugins.refresh(true);" href="po_pdf.php?level=original&pono=<?php echo $ctr['poNumber']; ?> " target="_blank"><img src="../../Images/s_f_prnt.gif" onclick="javascript:navigator.plugins.refresh(true);" alt="Print" width="17" border="0" title="Print PO Record" /></a></td>
        <td align="center">
          <?php echo $ctr['poDate']; ?>
        </td>
        <td>
