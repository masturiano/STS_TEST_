<?php
	...
	for ($j=1;$j<=$m_page;$j++){ 
		...
		$pdf->Cell($m_width_3_fields,5,"Report ID : SALES001R",0,0);
		$pdf->Cell($m_width_3_fields,5,"SALES TRANSACTIONS REGISTER",0,1,'C');
		//$pdf->Cell($m_width_3_fields,5,$entries,0,1,'R');
		if ($by_search=="by_product") {
			$pdf->Cell($m_width_3_fields,5,"",0,0);
			$pdf->Cell($m_width_3_fields,5,$from_to,0,1,'C');
			$pdf->ln();
			//$pdf->Cell(250, $dtl_ht, $from_to, 0, 1);
			//$pdf->ln();
			...
			$pdf->ln();
			$pdf->Cell(25,$dtl_ht, '', 0, 0);
			$pdf->Cell(30,$dtl_ht, 'UNIT', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, 'QTY', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, 'NET', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, '', 0, 0,'R');
			$pdf->ln();
			$pdf->Cell(25,$dtl_ht, 'DATE', 0, 0);
			$pdf->Cell(30,$dtl_ht, 'PRICE', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, 'SOLD', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, 'AMT', 0, 0,'R');
			$pdf->Cell(30,$dtl_ht, 'DISC', 0, 0,'R');
			$pdf->ln();
			$pdf->Cell($m_width, 0, '', 1, 0);
			$pdf->ln();
		} else {
			...
			$pdf->Cell(25,$dtl_ht, 'UNIT', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, 'QTY', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, 'NET', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, '', 0, 0,'R');
			$pdf->ln();
			$pdf->Cell(90,$dtl_ht, 'PRODUCT CODE AND DESCRIPTION', 0, 0);
			$pdf->Cell(25,$dtl_ht, 'PRICE', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, 'SOLD', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, 'AMT', 0, 0,'R');
			$pdf->Cell(25,$dtl_ht, 'DISC', 0, 0,'R');
			$pdf->ln();
			$pdf->Cell($m_width, 0, '', 1, 0);
			$pdf->ln();
		}
