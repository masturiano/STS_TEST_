...
function fPageLinks($limit_start = 0, $record_count = 0, $limit =0, $action="")
{
	...
	if ($limit_start > 0)
	{
		$first = "<img height='55%' src='../../Images/start_blu.gif' onClick=\"location='$PHP_SELF?limit_start=0$action'\"
					name=\"start\" onMouseOver=\"fChangeImg('start');\" onMouseOut=\"fRestoreImg('start');\" title='first'> ";
		$prev = "<img height='55%' src=\"../../Images/prev_blu.gif\" onClick=\"location='$PHP_SELF?limit_start=$prevrec$action'\"
					name=\"prev\" onMouseOver=\"fChangeImg('prev');\" onMouseOut=\"fRestoreImg('prev');\" title='previous'> ";
	}
	if ($record_count > ($limit_start + $limit))
	{
		$next = "<img height='55%' src=\"../../Images/next_blu.gif\" onClick=\"location='$PHP_SELF?limit_start=$nextrec$action'\"
					name=\"next\" onMouseOver=\"fChangeImg('next');\" onMouseOut=\"fRestoreImg('next');\" title='next'> ";
		$last = "<img height='55%' src=\"../../Images/end_blu.gif\" onClick=\"location='$PHP_SELF?limit_start=$val_last$action'\"
...
function pager_from($limit_start = 0, $record_count = 0, $limit =0, $action="")
{
	$lastrec = $limit_start;
	$from		= $limit_start+1;
	$tmp		= $limit_start+$limit;
