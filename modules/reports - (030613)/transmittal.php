...
switch($_GET['action']){	
	...
	case 'PrintAP':
		if($_GET['cmbTran']==5){
			echo "window.open('transmittal_PDF.php?{$_SERVER['QUERY_STRING']}');";
		}else{
			echo "window.open('transmittal_PDF.php?{$_SERVER['QUERY_STRING']}');";
		}
	exit();
	...
	case 'PrintAPXls':
		if($_GET['cmbTran']==5){
			echo "window.open('transmittal_XLS.php?{$_SERVER['QUERY_STRING']}');";
		}else{
			echo "window.open('transmittal_XLS.php?{$_SERVER['QUERY_STRING']}');";
		}
	exit();
...
$arrType = array("AP"=>"Accounts Payable","AR"=>"Accounts Receivable");
$arrTran = array("0"=>'ALL','1'=>"REGULAR STS",'2'=>"LISTING FEE",'3'=>"PROMO FUND",'4'=>"SHELF ENHANCER",'5'=>"DISPLAY ALLOWANCE");
$arrComp = array("ALL"=>'ALL','PPCI'=>"PPCI",'PG-JR'=>"PG-JR");
?>
...
<div class="ui-widget-content" style="padding:5px;">
	<table width="100%" border="0" cellspacing="3" cellpadding="2">
    	...
    	<tr>
        	<td class="hd" ><strong>Payment Type: </strong></td>
            ...
            <td class="hd" ><strong>Company: </strong></td>
           	<td><? $reportsObj->DropDownMenu($arrComp,'cmbComp','','class="selectBox"'); ?></td>
        ...
        <tr><td></td></tr>
       <tr>
        	<td colspan="8" align="center"><button id="printInq">Print in PDF</button> <button id="printInqXls">Print in EXCEL</button></td>
       </tr> 
