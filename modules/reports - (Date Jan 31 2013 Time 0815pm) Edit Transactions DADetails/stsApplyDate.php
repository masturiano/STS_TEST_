...
switch($_GET['action']){	
	...
	case 'PrintBR':
		echo "window.open('released_sts_excel_by_branch.php?{$_SERVER['QUERY_STRING']}');";
	exit();
	break;
	
	case 'PrintSUP':
		echo "window.open('released_sts_excel_by_supp.php?{$_SERVER['QUERY_STRING']}');";
	exit();
	break;
}
$arrStatus = array("",'A'=>"APPLIED",'O'=>"ON QUEUE");
$arrGrp = array('BR'=>"BRANCH",'SUP'=>"SUPPLIER");
	...
	$(function(){
		...
		$("#printInq").click( function (){
			...
			if (valDateStartEnd(dateStart,dateEnd,'txtDateFrom','txtDateTo')){
				$.ajax({
					url: 'stsApplyDate.php',
					type: "GET",
					data: $("#formInq").serialize()+'&action=Print'+$("#cmbGroup").val(),
...
function validateInputs() {
		var val = true;
		if (validateCmb($("#cmbStatus"))== false)
			val = false;
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">STS REPORT</h2>
<div class="ui-widget-content" style="padding:5px;">
	<table width="100%" border="0" cellspacing="3" cellpadding="2">
    	...
    	<tr>
            ...
            <td><? $reportsObj->DropDownMenu($arrStatus,'cmbStatus','','class="selectBox"'); ?></td>
            <td class="hd">Group by:</td>
            <td><? $reportsObj->DropDownMenu($arrGrp,'cmbGroup','','class="selectBox"'); ?></td>
        </form>
        
        	<td colspan="2" align="center"><button id="printInq">Print Report</button></td>
        </tr>
