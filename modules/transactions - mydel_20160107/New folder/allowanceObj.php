		...
		if((int)$_SESSION['sts-strCode']!=901){
			...
			$sql = "SELECT      CAST(tblBranches.strCode AS nvarchar) + ' - ' + tblBranches.brnDesc AS brnDesc, tblBranches.strCode, tblBranches.compCode,
                          ...
                          (SELECT     isnull(stsVatAmt,0)
							...
							(SELECT     eventNo
                            FROM          tblstsdtl
                            WHERE      compCode = tblBranches.compCode AND strCode = tblBranches.strCode AND stsRefNo = $refNo) AS eventNo,
							(SELECT     eventType
                            FROM          tblstsdtl
                            WHERE      compCode = tblBranches.compCode AND strCode = tblBranches.strCode AND stsRefNo = $refNo) AS eventType
	...
	function AddSTSDtl($arr){
		...
		$trans = $this->beginTran();
			...
			for($i=0;$i<=(float)$arr['hdCtr'];$i++) {
				if($arr["txt_$i"]!=""){
					 $sqlPar = "Insert Into tblStsDtl (stsRefno, compCode, strCode, stsAmt, dtlStatus, stsVatAmt, eventNo, eventType) 
					VALUES ('{$arr['hdDtl_refNo']}', '".$arr["comp_$i"]."','".$arr["ch_$i"]."','".$arr["txt_$i"]."','O','".$arr["txtVatAmt_$i"]."', '".$arr["event_$i"]."', '".$arr["hdnEventType_$i"]."');";
					if ($trans) {
						$stsAmt += $arr["txt_$i"];
