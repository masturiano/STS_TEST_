...
class PDF extends FPDF
{
	function Main($arr){
		...
		foreach($arr as $val){
			...
			$this->Cell(5,5,"",0,0,'L');
			$this->Cell(9,5,$val['stsNo'],0,0,'R');
			$this->Cell(5,5,"",0,0,'L');
			$this->Cell(20,5,date('m/d/Y',strtotime($val['applyDate'])),0,0,'L');	
			$this->Cell(20,5,date('m/d/Y',strtotime($val['endDate'])),0,0,'L');	
			
			if (strlen($val['suppName'])>15)
				$this->Cell(37,5,substr($val['suppName'],0,15)."...",0,0,'L');
			else
				$this->Cell(37,5,$val['suppName'],0,0,'L');	
			
			$this->Cell(38,5,$val['paymentMode'],0,0,'L');
	...
	function Header() {
		...
		$this->Cell(55,5,"Cancelled STS Transactions",0,0,'L');
...
$arr = $reportsObj->getCancelledSTS($_GET['cmbCompCode'],$_GET['cmbProdGrp'],$_GET['txtDateFrom'],$_GET['txtDateTo']);
$pdf->Main($arr);
$pdf->Output('releasedSTS.pdf','D');
