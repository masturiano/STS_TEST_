		...
		$sql = "SELECT TOP  $limit
			...
			FROM
			  tblStsHdr INNER JOIN
                      APSUPP ON tblStsHdr.suppCode = APSUPP.ASNUM
	...
	function getRegSTSInfoAssoc($refNo){
		$sql = "
            SELECT     
                ...
                tblStsHdr.applyDate, 
                tblStsHdr.contactPerson, 
                tblStsHdr.contactPersonPos, 
                sql_mmpgtlib.dbo.APSUPP.ASNAME as suppName, 
                tblStsHdr.endDate, 
                tblStsHdr.vatTag, 
                tblStsHdr.impStartDate, 
                tblStsHdr.impEndDate
		    FROM
                tblStsHdr
            INNER JOIN
                sql_mmpgtlib.dbo.APSUPP ON tblStsHdr.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM 
			...
			for($i=0;$i<=(int)$arr['hdCtr'];$i++) {
				if($arr["switcher_$i"]=="1"){
					$sqlPar = "
                        Insert Into 
                            tblStsDaDetail 
                            (stsRefNo, compCode, strCode, displayType, brand, 
                            location, daSize, dispSpecs, noUnits, daRemarks, 
                            perUnitAmt, stsAmt, stsVatAmt, stsEwtAmt) 
...
FROM         tblStsHdr INNER JOIN
	...
	function getEnhancerDetails($refNo){
		$sql = "
            SELECT     
                tblBranches.brnDesc AS brnDesc, 
                tblStsDaDetail.stsRefno, 
                tblStsDaDetail.compCode, 
                tblStsDaDetail.strCode, 
                tblStsDaDetail.location, 
                tblStsDaDetail.daSize, 
                tblStsDaDetail.dispSpecs, 
                tblStsDaDetail.noUnits, 
                tblStsDaDetail.daRemarks, 
                tblStsDaDetail.contractNo, 
                tblStsDaDetail.stsNo, 
                tblStsDaDetail.perUnitAmt, 
                tblStsDaDetail.stsAmt, 
                tblDisplaySpecs.displaySpecsDesc AS displayType, 
                tblStsDaDetail.brand
            FROM
                tblStsDaDetail
            INNER JOIN
                tblBranches ON tblBranches.strCode = tblStsDaDetail.strCode 
            INNER JOIN
                      tblDisplaySpecs ON tblStsDaDetail.dispSpecs  = tblDisplaySpecs.displaySpecsId 
            WHERE 
                tblStsDaDetail.stsRefno = '$refNo' 
            ORDER BY 
                tblStsDaDetail.strCode";	
		return $this->getArrRes($this->execQry($sql));
	}
