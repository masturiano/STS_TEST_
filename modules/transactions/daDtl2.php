...
switch($_POST['action']) {
        ...
        <script type="text/javascript">   
            ...
            // add multiple select / deselect functionality
                $("#chkAll").click(function () {
                    ...
                    var vatable = $('#txtVatable').val();
                    var vatAmt = $('#hdnVat').val();
                    var ewtAmt = $('#hdnEwt').val();
                    ...
                    if(counter > 0){
                        $(function(){
                            for(a=0;a<counter;a++) {
                                ...
                                if(vatable == 'Y'){
                                    var vatAmount = displayFee * vatAmt; 
                                    vatAmount = parseFloat(vatAmount).toFixed(2);
                                    var ewtAmount = displayFee * ewtAmt * -1; 
                                    ewtAmount = parseFloat(ewtAmount).toFixed(2);
                                ...
                                $("#txtEwtAmount_"+chkArray[a]).val(ewtAmount);    
    ...
    ?>      
        <table width="100%" border="1" cellspacing="0" cellpadding="0" class="details"
            ...
            <tr>
                <td width="1%" height="25">
                    <input type="checkbox" name="chk_<?=$i;?>" id="chk_<?=$i;?>"  value="<?=$i;?>" class="case">
                    <input type="hidden" name="txtDisplaySpecsIdSeriesStore_<?=$i;?>" id="txtDisplaySpecsIdSeriesStore_<?=$i;?>" value="<?=$val['displaySpecsIdSeriesStore'];?>"/>
                    <input type="hidden" name="txtSizeSpecs_<?=$i;?>" id="txtSizeSpecs_<?=$i;?>" value="<?=$val['sizeSpecs'];?>"/>
                </td>
                <td align="center">
                    <?=$val['strCode'];?>
                    <input type="hidden" name="txtStoreCode_<?=$i;?>" id="txtStoreCode_<?=$i;?>" value="<?=$val['strCode'];?>"/>
                    <input type="hidden" name="txtCompCode_<?=$i;?>" id="txtCompCode_<?=$i;?>" value="<?=$val['compCode'];?>"/>
                </td>
                <td align="center"><?=$val['suffixSeries'];?></td>
                <td align="center">
                    <input type="text" name="txtDisplayFee_<?=$i;?>" id="txtDisplayFee_<?=$i;?>" class="textBoxTemp case" value="<?=$val['specsAmount'];?>">
                </td>
                <td align="center">
                    <input type="text" name="txtPerMonth_<?=$i;?>" id="txtPerMonth_<?=$i;?>" class="textBoxTemp case" value="" style="background-color: lightgray;" readonly>
                </td>
                <?php
                if($_POST['with_vat'] == 'Y'){
                ?>  
                    <td align="center">
                        <input type="text" name="txtVatAmount_<?=$i;?>" id="txtVatAmount_<?=$i;?>" class="textBoxTemp case" value="" style="background-color: lightgray;" readonly> 
                        <input type="hidden" name="txtEwtAmount_<?=$i;?>" id="txtEwtAmount_<?=$i;?>" value="">       
                    </td>    
                <?php
                }
                else{
                ...
                ?>
                    <td align="center">
                        <input type="text" name="txtVatAmount_<?=$i;?>" id="txtVatAmount_<?=$i;?>" class="textBoxTemp case"  value="<?=$vat_amount;?>" style="background-color: lightgray;" readonly>    
                        <input type="hidden" name="txtEwtAmount_<?=$i;?>" id="txtEwtAmount_<?=$i;?>" value=""> 
                    </td>    
                <?php       
                }
                ?>   
                <td align="center">
                    <input type="text" name="txtGrossAmount_<?=$i;?>" id="txtGrossAmount_<?=$i;?>" class="textBoxTemp case" style="background-color: lightgray;" readonly>    
                </td>
            </tr>
            <?php
            $i++;
        ...
        <input type="hidden" name="txtCounter" id="txtCounter" value="<?=($i-1);?>"/>
        <input type="hidden" name="txtCounterId" id="txtCounterId" value=""/>
    <?php            
    exit();
    break; 
        ...
        <legend>TEMPLATE</legend>
            <table width="100%" border="0" cellspacing="0" cellpadding="2" align="center">
                <tr> 
                    ...
                    <td>
                        <input type="text" name="txtNoUnits" id="txtNoUnits" class="textBoxTemp" onChange="computeMonthly();" onKeyPress='return valNumInputDec(event); ' style="text-align:right; background-color: lightgray;" readonly/>
                    </td>    
                    <td class="hd2">Display Fee per Unit</td>
                    <td>
                        <input type="text" name="txtUnitAmt" id="txtUnitAmt" class="textBoxTemp" onKeyPress='return valNumInputDec(event);' style="text-align:right; background-color: lightgray;" onChange="computeMonthly();" readonly/>
                    </td>
                <tr/>
                <tr> 
