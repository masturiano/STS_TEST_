		...
		$arrTran = $reportsObj->transSummarySupp($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$_GET['cmbSupp']);
			...
			foreach ($arrTran as $val) {
				...
				foreach($arrH as $valD){
					...
					$arrDet = $reportsObj->getParDetailDa($valD['stsRefno']);
					$ctr++;
					$worksheet->write($ctr,3,"DA NO.",$row);
					$worksheet->write($ctr,4,"BRANCH",$row2);
					$worksheet->write($ctr,5,"DISPLAY SPECS",$row2);
					$worksheet->write($ctr,6,"STORE AMOUNT",$row2);
					
					foreach ($arrDet as $valDet) {
						$ctr++;
						$worksheet->write($ctr,3,$valDet['stsNo'],$row);
						$worksheet->write($ctr,4,$valDet['strCode']." - ".$valDet['brnShortDesc'],$row);
						$worksheet->write($ctr,5,$valDet['displaySpecsDesc'],$row);
