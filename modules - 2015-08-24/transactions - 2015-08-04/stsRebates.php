...
include("rebatesObj.php");
$rebatesObj = new rebatesObj();
...
switch($_GET['action']){	
	case 'Load':
		...
		if($_GET['_search']=='true'){
			$arrSTS= $rebatesObj->searchRebatesSTS($sidx,$sord,$start,$limit,$_GET['searchField'],$_GET['searchString']);
		}else{
			if(isset($_GET['compCode'])){
				$arrSTS = $rebatesObj->getPaginatedRebatesSTSComp($sidx,$sord,$start,$limit,$_GET['compCode']);
			}else{
				$arrSTS = $rebatesObj->getPaginatedRebatesSTS($sidx,$sord,$start,$limit);
			}
		}
		$i = 0;
	...
	case 'searchSupplier':
		$arrResult = array();
		$arrSupp = $rebatesObj->findSupplier($_GET['term']);
	...
	case 'cmbClass':
		$rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->findClass($_GET['dept']),'stsTransTypeClass','stsTransTypeName',''),'cmbClass','','class="selectBox" onchange="loadSubClass(this.value);"');
	exit();
	break;
	
	case 'cmbSubClass':
		$rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->findSubClass($_GET['dept'],$_GET['class']),'stsTransTypeSClass','stsTransTypeName',''),'cmbSubClass','','class="selectBox" ');
	exit();
	break;
	
	case 'cmbStore':
		$rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->findStore($_GET['compCode']),'brnCode','brnDesc',''),'cmbStore','','class="selectBox" ');
	exit();
	break;
	case 'saveHdr':
		if($rebatesObj->saveHeader($_GET)){
			$lastQuery = $rebatesObj->getLastSTSInserted();
	...
	case 'getInfo':
		$val = $rebatesObj->getRegSTSInfoAssoc($_GET['refNo'],$_GET['compCode']);
		$supp = $rebatesObj->getSuppName($val['suppCode']);
	...
	case 'LoadSTSDetails':
		$arr = $rebatesObj->getRegSTSDetails($_GET['refNo'],$_GET['compCode']);
		if($arr[0]['brnDesc']==''){
	...
	case 'UpdateHdr':
		if($rebatesObj->updateHeader($_GET)){
	...
	case 'DeleteSTS':
		if($rebatesObj->DeleteSTS($_GET['refNo'],$_GET['compCode'])) {
	...
	case 'ReleaseSTS':
		$count = $rebatesObj->hasSTSDetail($_GET['refNo'],$_GET['compCode']);
		if((int)$count>0){
			if($rebatesObj->releaseSTS($_GET['refNo'],$_GET['compCode'])){
				echo "$('#dialogAlert').dialog('close');\n";
...
$(function(){
	...
	$("#stsTable").jqGrid({ 
		url:'stsRebates.php?action=Load', 
		datatype: "json", 
	...
	$("#txtSupp").autocomplete({
		source: "stsRebates.php?action=searchSupplier",
		minLength: 1,
...
function searchGridCompany(compCode){
	$("#stsTable").jqGrid('setGridParam',{url:"stsRebates.php?action=Load&compCode="+compCode,page:1}).trigger("reloadGrid");
}
...
function EditSTS(){
	...
	//LoadData('stsRebates.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+refNo+'&compCode='+compCode);
	$("#dialogAdd").dialog("destroy");
	$("#dialogAdd").dialog({
...
<h2 class=" ui-widget-header">REBATES</h2>
<br />
...
<? $rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->getAllCompany(),'compCode','compShortName',''),'cmbCompCodeMain','','class="selectBox" onchange="searchGridCompany(this.value);"'); ?>
</div>
<br /><br />
        ...
        <div id='dialogAdd' title='Add'>
            ...
            <table width="100%" border="0" cellspacing="2" cellpadding="0" align="center">
                <tr>
                    ...
                    <td><? $rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->getAllCompany(),'compCode','compShortName',''),'cmbCompCode','','class="selectBox"'); ?></td>
                ...
                <tr>
                    ...
                    <td><input type="text" name="txtSupp" id="txtSupp" class="textBox"/>
                    
                    </td>
                    <td class="hd">Payment Mode: </td> 
                    <td><? $rebatesObj->DropDownMenu($arrMode,'cmbPayType','','class="selectBox"'); ?></td>
                </tr>
                <tr >
                	<td class="hd">Supplier's Currency: </td>
                    <td><? $rebatesObj->DropDownMenu($rebatesObj->makeArr($rebatesObj->distinctSuppCur(),'suppCurr','suppCurr',''),'cmbSuppCurr','','class="selectBox"'); ?></td>
