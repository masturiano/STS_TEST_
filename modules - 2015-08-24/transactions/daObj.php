...
WHERE     (tblStsDtl.stsRefno = $refNo)";
	...
	function untagRentables($refNo,$strCode){
        ...
        $sql3 = "Delete FROM tblStsDaDetail WHERE stsRefNo = '$refNo' and strCode = '$strCode' ";
        if ($trans) {
            $trans = $this->execQry($sql3);
        }
        $sql4 = "Delete FROM tblStsDtl WHERE stsRefNo = '$refNo' and strCode = '$strCode' ";
        if ($trans) {
            $trans = $this->execQry($sql4);
        }
        if ($trans) {
            $trans = $this->execQry($sql3);
        }
				...
				if($this->deleteRentablesStr($arr['refNo'],$val['strCode'],$val['dispSpecs'])){
					...
					foreach($arrDispStr as $valStr){
						...
						if((int)$arr["hdnSwitcherRentables_$concatenated"]==1){
                            ...
                            $sql1 = "Insert into tblDispDaDtlStr (strCode, displaySpecsId, dispDtlId, stsRefNo, startDate, endDate, enteredBy, entryDate, status) VALUES ('".$val['strCode']."', '".$val['dispSpecs']."', '".$valStr["dispDtlId"]."', '".$arr["refNo"]."', '".$arrHdr['impStartDate']."', '".$arrHdr['impEndDate']."','".$_SESSION['sts-userId']."','".date('m/d/Y')."', '$status')";
                            if ($trans) {
                                $trans = $this->execQry($sql1);
                            }
    ...
    function hasSTSDaDtlStr($refNo){
        $sql = "SELECT * FROM tblDispDaDtlStr WHERE stsRefno = '$refNo'";    
        return $this->getRecCount($this->execQry($sql));
    }
     function hasSTSDaDtlStrHist($refNo){
        $sql = "SELECT * FROM tblDispDaDtlStrHist WHERE stsRefno = '$refNo'";    
        return $this->getRecCount($this->execQry($sql));
    }
	...
	function updateHeader($arr){
        ...
        $countDaDtl = $this->hasSTSDaDtlStr($arr['refNo']);
        # Mydel - For display allowance (Update tblDispDaDtlStr sync impstartdate and impenddate from header)
        if((int)$countDaDtl>0){        
            //$arrParDtl = $this->getStsDaDtlArr($arr['refNo']);
                //foreach($arrParDtl as $val) {
                     $sqlParDtlSts = "update tblDispDaDtlStr SET 
                                     startDate = '".$arr['txtImStartDate']."',
                                     endDate = '".$arr['txtImEndDate']."' 
                                     where stsRefno = '{$arr['refNo']}'\n";
                    if ($trans) {
                    $trans = $this->execQry($sqlParDtlSts);
                    }
               // }
        }
            # Update hist also
        $countDaDtlHist = $this->hasSTSDaDtlStrHist($arr['refNo']);
        if((int)$countDaDtlHist>0){        
            //$arrParDtl = $this->getStsDaDtlArr($arr['refNo']);
                //foreach($arrParDtl as $val) {
                     $sqlParDtlStsHist = "update tblDispDaDtlStrHist SET 
                                     startDate = '".$arr['txtImStartDate']."',
                                     endDate = '".$arr['txtImEndDate']."' 
                                     where stsRefno = '{$arr['refNo']}'\n";
                    if ($trans) {
                    $trans = $this->execQry($sqlParDtlStsHist);
                    }
               // }
        }
        # Mydel - For display allowance (Update tblDispDaDtlStr sync impstartdate and impenddate from header) END
		if ($trans) {
			$trans = $this->execQry($sqlUpdateHdr);
		}
...
FROM         tblStsHdr INNER JOIN
	...
	function getEnhancerDetails($refNo){
		$sql = "
            SELECT     tblBranches.brnDesc AS brnDesc, tblStsDaDetail.stsRefno, tblStsDaDetail.compCode, tblStsDaDetail.strCode, tblStsDaDetail.location, 
            ...
            FROM         tblStsDaDetail 
            INNER JOIN tblBranches ON tblBranches.strCode = tblStsDaDetail.strCode 
            INNER JOIN tblDisplaySpecs ON tblStsDaDetail.dispSpecs  = tblDisplaySpecs.displaySpecsId 
	...
	function getDaContract($refNo,$strCode){
		...
		tblStsDaDetail.noUnits, tblStsDaDetail.daRemarks,
					 ...
					 sql_mmpgtlib.dbo.APADDR.AAADD1 AS add1, 
                      ...
                      tblStsHdr.stsRefno,tblStsHdr.impStartDate,tblStsHdr.impEndDate,
...
FROM         tblDispDaDtlStr INNER JOIN
	...
	function getRentableDtlsTactical($refNo,$dispSpecs,$strCode,$dateFrom,$dateTo){
        ...
        $dateToPlusOne = date("m/d/Y", strtotime("$dateTo +1 day", time()));
		  # Mydel add filtering = and stsRefno is null
        $sqlCount = "
        SELECT  tblDispDaDtlStr.*, tblLocation.locDescription as locDescription
        FROM    tblDispDaDtlStr 
        LEFT JOIN tblLocation ON 
            tblDispDaDtlStr.locId = tblLocation.locId
		...
		if((int)$recCountDtlStr > 0){
			$sqla = "
                SELECT  tblDisplaySpecsDtl.dispDesc, tblDisplaySpecs.displaySpecsDesc, tblDispDaDtlStr.dispDtlId, tblDispDaDtlStrHist.stsRefNo, 
                tblLocation.locDescription as locDescription
                FROM    tblDispDaDtlStr 
                INNER JOIN  tblDisplaySpecsDtl 
                    ON tblDispDaDtlStr.dispDtlId = tblDisplaySpecsDtl.dispDtlId 
                INNER JOIN  tblDisplaySpecs 
                    ON tblDisplaySpecsDtl.displaySpecsId = tblDisplaySpecs.displaySpecsId 
                LEFT OUTER JOIN tblDispDaDtlStrHist 
                    ON tblDispDaDtlStr.strCode = tblDispDaDtlStrHist.strCode 
                    AND tblDispDaDtlStr.displaySpecsId = tblDispDaDtlStrHist.displaySpecsId 
                    AND tblDispDaDtlStr.dispDtlId = tblDispDaDtlStrHist.dispDtlId
                LEFT JOIN   tblLocation 
                    ON tblDispDaDtlStr.locId = tblLocation.locId
                WHERE     tblDisplaySpecsDtl.displaySpecsId = '$dispSpecs' 
                AND tblDispDaDtlStr.strCode = '$strCode' 
                AND tblDispDaDtlStr.permanentTag = 'Y' 
                AND tblDispDaDtlStr.stsRefNo = '".$refNo."'";
			...
			if((int)$resCountSqla > 0){
				...
				$sql = "
                    SELECT   top $recCountDtlStr  tblDisplaySpecsDtl.dispDesc, tblDisplaySpecs.displaySpecsDesc, tblDispDaDtlStr.dispDtlId, 
                    tblLocation.locDescription as locDescription
                    FROM         tblDispDaDtlStr 
                    INNER JOIN tblDisplaySpecsDtl 
                        ON tblDispDaDtlStr.dispDtlId = tblDisplaySpecsDtl.dispDtlId 
                    ...
                    WHERE     tblDisplaySpecsDtl.displaySpecsId = '$dispSpecs' 
                        AND tblDispDaDtlStr.strCode = '$strCode' 
                        AND tblDispDaDtlStr.permanentTag = 'Y' 
                        AND (((tblDispDaDtlStrHist.stsRefNo is null) OR (tblDispDaDtlStrHist.endDate < '".$dateToPlusOne."')) OR
                            ((tblDispDaDtlStr.stsRefNo is null) OR (tblDispDaDtlStr.endDate < '".$dateToPlusOne."'))) 
			...
			FROM tblDispDaDtlStr 
            INNER JOIN tblDisplaySpecsDtl ON 
			    tblDispDaDtlStr.dispDtlId = tblDisplaySpecsDtl.dispDtlId 
            INNER JOIN tblDisplaySpecs ON 
