...
class PDF extends FPDF
{
	...
	function Main()
	{
		...
		$reportsObj = new reportsObj();
			$this->Ln(3);
				...
				$arrH = $reportsObj->stsSummary($this->tran,$this->dtFrom,$this->dtTo,$this->group,$this->suppCode);
				foreach ($arrH as $valH) {
					$ctr++;
					$this->Cell(15,5,$valH['stsRefno'],0,0,'L');
					$this->Cell(20,5,$valH['stsNo']."-".$valH['nbrApplication'],0,0,'L');
					...
					$this->Cell(4,5,'',0,0,'L');
					$this->Cell(20,5,date('m/d/Y',strtotime($valH['dateApproved'])),0,0,'L');
	...
	function Header() {
		...
		$this->Cell(198,5,'Approved STS Summary Report',0,0,'C');
		...
		$this->Cell(25,8,'Date Approved','BT',0,'L');
...
$pdf->grpName = $grpName;
$pdf->Main();
$pdf->Output('approved_STS_Summary.pdf','D');
