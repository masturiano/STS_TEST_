...
FROM         tblDisplaySpecs order by displaySpecsDesc";
	...
	function specsDetail($strCode,$dispSpecs,$availTag,$impDteFrm,$impDteTo){
		...
		tblDisplaySpecsDtl.status, tblUsers.fullName,
		tblStsHdr.impStartDate,tblStsHdr.impEndDate,
		tblSizeSpecs.sizeSpecsDesc,
		tblGroup.grpDesc, tblStsHdr.stsRefno
		FROM tblDispDaDtlStr 
		LEFT OUTER JOIN tblDisplaySpecsDtl ON tblDispDaDtlStr.dispDtlId = tblDisplaySpecsDtl.dispDtlId 
		LEFT OUTER JOIN tblDisplaySpecs ON tblDispDaDtlStr.displaySpecsId = tblDisplaySpecs.displaySpecsId 
		LEFT OUTER JOIN tblSizeSpecs ON  tblSizeSpecs.sizeSpecsId = tblDisplaySpecs.displaySpecsId
		LEFT OUTER JOIN tblBranches ON tblBranches.strCode = tblDispDaDtlStr.strCode 
		LEFT OUTER JOIN tblUsers ON tblUsers.userId = tblDisplaySpecsDtl.createdBy 
		LEFT OUTER JOIN tblStsHdr ON cast(tblStsHdr.stsRefno as nvarchar) = tblDispDaDtlStr.stsRefNo
		LEFT OUTER JOIN tblGroup ON tblGroup.grpCode = tblDispDaDtlStr.grpCode
		WHERE $filterAvailTag
		$filterStrCode
		$filterDispSpecs
		order by tblDispDaDtlStr.strCode
		";
		return $this->getArrRes($this->execQry($sql));
	...
	function specsDetailAll($strCode){
		...
		and  (tblDispDaDtlStr.permanentTag IS NOT NULL)
		$filterStrCode
