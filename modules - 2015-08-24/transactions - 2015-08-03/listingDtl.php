...
switch($_POST['action']) {
	...
	break;
	case "Delete":
			if($listObj->DeleteSTSDtl($_POST['refNo'])) {
 ...
 function CopyTxt(objSource,objClip) {
	...
	function ToggleBranch(id) {
		...
		if (chckBox.attr('checked')) {
			...
			$('#txtVatAmt_'+id).val(addDecimal(parseFloat(defaultVal)*parseFloat(vat)));
			$('#txtEwtAmt_'+id).val(addDecimal(defaultVal*ewt*-1));
			$('#txtSubAmt_'+id).val(addDecimal(parseFloat(defaultVal)+(defaultVal*vat)));
	...
	function checkAllDtl() {
		...
		if ($('#chAll').attr('checked')) {
			for(i=0;i<=chldCnt;i++){
				...
				$('#txtVatAmt_'+i).val(addDecimal(parseFloat(defaultVal)*parseFloat(vat)));
				$('#txtEwtAmt_'+i).val(addDecimal(defaultVal*ewt*-1));
				$('#txtSubAmt_'+i).val(addDecimal(parseFloat(defaultVal)+(defaultVal*vat)));
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            foreach($arrBranches as $val) {
				...
				$readonly2 = ($val['stsAmt'] != '') ? "value=".$val['stsVatAmt']:" readonly";
				$readonly3 = ($val['stsAmt'] != '') ? "value=".$val['stsEwtAmt']:" readonly";
