...
class PDF extends FPDF
{
	...
	function Content() {
		...
		$dtlEvent = $transObj->checkEventOnDetail($_GET['refNo']);
		if($dtlEvent['eventNo']==''){
			##########################################################
			//Column widths
			$w=array(30,50,35);
			...
			foreach($arrStsPar as $row){
				...
				$this->Cell($w[1],6,$row['strCode']." - ".$row['brnShortDesc'],'LR',0,'L');
				$this->Cell($w[2],6,number_format($row['stsAmt'],2),'LR',0,'R');
				$this->Cell($w[3],6,$row['eventNo']." - ".$row['eventType'],'LR',0,'L');
				$this->Ln();
			}
			//Closure line
			$arrSTS = $transObj->getSTSPrint($_GET['refNo']);
	...
	function Header() {
		...
		if($arrSTS['eventNo']!=''){
			$this->Cell(25,5,'EVENT NO: ',0,0,'L');
			$this->Cell(20,5,$arrSTS['eventNo'],0,0,'L');
			$this->Cell(75,5,"",0,0,'C');
			$this->Cell(38,5,'EVENT TYPE: ',0,0,'L');
			$this->Cell(20,5,$arrSTS['eventType'],0,1,'L');
		}
		...
		if($dtlEvent['eventNo']==''){
			...
			$w=array(30,50,35,35);
			//Header
			$header=array('STS No','Store','Amount','Event');
			$this->Cell(20,6,"",0,0,'L');	
			for($i=0;$i<count($header);$i++)
				$this->Cell($w[$i],7,$header[$i],1,0,'C');	
		}
