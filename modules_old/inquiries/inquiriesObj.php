...
class inquiriesObj extends commonObj {
	...
	function getSTSHdr($no){
		$sql = "SELECT     tblStsHdr.stsRefno, tblStsHdr.suppCode, pg_pf.dbo.tblSuppliers.suppName, tblStsHierarchy.hierarchyDesc, tblStsHdr.stsAmt, tblStsHdr.stsRemarks, tblStsHdr.nbrApplication, 
                      ...
                      tblStsHdr.nbrApplication - 1, tblStsHdr.applyDate) AS endDate, 
                      payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END, tblStsHdr.contractNo
...
FROM         tblStsHdr INNER JOIN
	...
	function checkContractNo($no){
		$sql = "SELECT contractNo FROM tblStsHdr WHERE contractNo = '$no'";
...
FROM         tblStsApply LEFT OUTER JOIN
				...
				WHERE tblStsHdr.contractNo = '$contractNo' $qry ";
		$amt =  $this->getSqlAssoc($this->execQry($sql));	
	}
	function getSTSDet($no){
		$sql ="SELECT     tblStsDtl.stsRefno, tblStsDtl.stsNo, tblStsHdr.dateApproved, tblStsHdr.applyDate, tblStsHdr.nbrApplication, tblStsHdr.suppCode, pg_pf.dbo.tblBranches.brnShortDesc, 
                      tblStsDtl.stsAmt, tblStsHierarchy.hierarchyDesc, tblStsHdr.stsRemarks, tblStsHdr.stsPaymentMode, tblUsers.fullName, DATEADD(month, tblStsHdr.nbrApplication - 1, 
                      tblStsHdr.applyDate) AS endDate, tblStsHdr.contractNo, sql_mmpgtlib.dbo.APSUPP.ASNAME AS suppName,
...
FROM         tblStsDtl INNER JOIN
                      ...
                      sql_mmpgtlib.dbo.APSUPP ON tblStsHdr.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM WHERE tblStsDtl.stsNo = '$no' and tblStsHierarchy.levelCode = 1";	
		return $this->getSqlAssoc($this->execQry($sql));	
	}
