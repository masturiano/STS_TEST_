...
class listObj extends commonObj {
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsRemarks, 
			...
			VALUES 
			('$tempRefNo', '{$arr['hdnSuppCode']}', '1', '0', '0', '{$arr['txtRemarks']}', '{$arr['cmbPayType']}', NULL, '1', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-grpCode']."', 'O', '2', 'Y', '{$arr['txtRep']}', '{$arr['txtRepPos']}','".$_SESSION['sts-strCode']."')";
...
WHERE     (tblStsDtl.stsRefno = $refNo)";
	...
	function updateHeader($arr){
		...
		$sqlUpdateDtl = "UPDATE tblStsDtl set stsAmt = '{$arr['txtAmtPerStr']}' WHERE stsRefno = '{$arr['refNo']}'";
		$trans = $this->beginTran();
		if ($trans) {
...
FROM         tblStsHdr LEFT OUTER JOIN
	...
	function getRegStsParticipants($refNo){
		$sql = "SELECT     tblStsDtl.stsRefno, tblStsDtl.compCode, tblStsDtl.strCode, tblStsDtl.stsAmt, tblStsDtl.stsNo, pg_pf.dbo.tblBranches.brnShortDesc
FROM         tblStsDtl INNER JOIN
                      pg_pf.dbo.tblBranches ON tblStsDtl.compCode = pg_pf.dbo.tblBranches.compCode AND tblStsDtl.strCode = pg_pf.dbo.tblBranches.strCode
					  WHERe stsRefno = '$refNo'";	
		return $this->getArrRes($this->execQry($sql));	
