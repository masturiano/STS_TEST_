...
class daObj extends commonObj {
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsRemarks, 
			...
			'{$arr['txtNoApplications']}', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-grpCode']."', 'O', '5', 'Y', '{$arr['txtRep']}', '{$arr['txtRepPos']}', '{$arr['txtEndDate']}','".$_SESSION['sts-strCode']."')";
	...
	function getRegSTSInfoAssoc($refNo){
		$sql = "SELECT     tblStsHdr.stsRefno, tblStsHdr.suppCode, tblStsHdr.stsAmt, tblStsHdr.stsRemarks, tblStsHdr.stsPaymentMode, tblStsHdr.nbrApplication, tblStsHdr.applyDate, tblStsHdr.contactPerson, tblStsHdr.contactPersonPos, sql_mmpgtlib.dbo.APSUPP.ASNAME as suppName, tblStsHdr.endDate
	...
	function getRegSTSDetails($refNo){
		$sql = "SELECT     tblStsDtl.stsRefno, tblStsDtl.compCode, tblStsDtl.strCode, tblStsDtl.stsAmt, tblStsDtl.enhanceType, pg_pf.dbo.tblBranches.brnShortDesc, pg_pf.dbo.tblBranches.brnDesc
FROM         pg_pf.dbo.tblBranches INNER JOIN
	...
	function getAllBranches(){
		...
		$sql = "SELECT strCode+'|'+compCode strCodeComp, strCode +' - '+ brnShortDesc as strCodeBranch FROM pg_pf.dbo.tblBranches $filter2";
		return $this->getArrRes($this->execQry($sql));
	}
		...
		$trans = $this->beginTran();
			for($i=0;$i<=(int)$arr['hdCtr'];$i++) {
				if($arr["switcher_$i"]=="1"){
					$sqlPar = "Insert Into tblStsDaDetail (stsRefNo, compCode, strCode, displayType, brand, location, daSize, dispSpecs, noUnits, daRemarks, perUnitAmt, stsAmt) 
					VALUES ('{$arr['hdDtl_refNo']}', '".$arr["comp_$i"]."','".$arr["ch_$i"]."', '".$arr['txtDispTyp']."', '".$arr['txtBrand']."','".$arr["txtLoc"]."','".$arr["txtSizeSpecs_$i"]."','".$arr["txtDispSpecs_$i"]."','".$arr["txtNoUnits_$i"]."','".$arr["txtRem"]."','".$arr["txtUnitAmt_$i"]."','".$arr["txtMonthly_$i"]."');";
					if ($trans) {
						$trans = $this->execQry($sqlPar);
					}
					$sqlDtl = "Insert Into tblStsDtl (stsRefno, compCode, strCode, stsAmt, dtlStatus) 
					VALUES ('{$arr['hdDtl_refNo']}', '".$arr["comp_$i"]."','".$arr["ch_$i"]."','".round($arr["txtMonthly_$i"]*$nbr,2)."','O');";
					if ($trans) {
...
FROM         tblStsHdr INNER JOIN
	...
	function getEnhancerDetails($refNo){
		$sql = "SELECT     pg_pf.dbo.tblBranches.brnDesc AS brnDesc, tblStsDaDetail.stsRefno, tblStsDaDetail.compCode, tblStsDaDetail.strCode, tblStsDaDetail.location, 
...
FROM         tblStsDaDetail INNER JOIN
                      pg_pf.dbo.tblBranches ON pg_pf.dbo.tblBranches.strCode = tblStsDaDetail.strCode INNER JOIN
...
FROM         tblStsEnhanceDtl INNER JOIN
	...
	function getDaContract($refNo,$strCode){
			$sql = "SELECT      tblStsDaDetail.contractNo, pg_pf.dbo.tblBranches.brnDesc, tblStsDtl.stsAmt, tblStsDtl.stsNo, sql_mmpgtlib.dbo.APADDR.AANUM,
		...
		tblStsDaDetail.noUnits, tblStsDaDetail.daRemarks,
					 ...
					 sql_mmpgtlib.dbo.APADDR.AAADD1 AS add1, 
                      ...
                      tblSizeSpecs.sizeSpecsDesc
		FROM         pg_pf.dbo.tblBranches INNER JOIN
                      tblStsDaDetail ON pg_pf.dbo.tblBranches.strCode = tblStsDaDetail.strCode INNER JOIN
                      ...
                      tblSizeSpecs ON tblStsDaDetail.daSize = tblSizeSpecs.sizeSpecsId
