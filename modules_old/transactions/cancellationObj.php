...
FROM         tblStsDtl INNER JOIN
	...
	function cancelSTSWHOLE($refNo,$reason,$cancelDate){
		...
		$sqlInsertCancelled = "INSERT INTO tblcancelledsts (stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate, stsStrAmt,cancelDate, cancelCode, cancelledBy) 
		SELECT     tblStsDtl.stsNo, '0' AS seq, tblStsDtl.stsRefno, tblStsDtl.compCode, tblStsDtl.strCode, tblStsHdr.suppCode, tblStsHdr.stsType, tblStsHdr.stsPaymentMode, 
                      tblStsHdr.stsDept, tblStsHdr.stsCls, tblStsHdr.stsSubCls, tblStsHdr.grpCode, tblStsHdr.applyDate,tblStsDtl.stsAmt,  '".date('m/d/Y',strtotime($cancelDate))."', '".$lastId."', '".$_SESSION['sts-userId']."'
FROM         tblStsDtl INNER JOIN
	...
	function cancelStsDtl($arr){
		for($i=0;$i<=(int)$arr['hdCtr2'];$i++) {
			if($arr["switcher_".$i]=="1"){
				...
				$trans = $this->beginTran();
				####### insert cancel Type
				$sqlInsertReason = "INSERT INTO tblCancelType (cancelDesc, cancelStat, createdBy, dateAdded) VALUES ('$reason', 'A', '".$_SESSION['sts-userId']."', '".date('m/d/Y',strtotime($cancelDate))."');";
				if($trans){
					$trans = $this->execQry($sqlInsertReason);	
				}
				if($trans){
				...
				$sqlInsertCancelled = "INSERT INTO tblcancelledsts (stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate,cancelledBy,cancelDate,cancelCode) SELECT stsNo, stsSeq, stsRefNo, compCode, strCode, suppCode, stsType, stsPaymentMode, stsDept, stsCls, stsSubCls, grpCode, stsApplyDate,  '".$_SESSION['sts-userId']."', '".$cancelDate."', '".$lastId."' FROM tblstsapply WHERE  stsRefno = '".$arr['hdnRefno']."' AND stsApplyDate >= '$cancelDate' AND stsNo = '".$arr['stsNo_'.$i]."';";
				if($trans){
					$trans = $this->execQry($sqlInsertCancelled);	
				}
				...
				$sqlDelStsApply = "DELETE FROM tblstsapply WHERE stsRefNo = '".$arr['hdnRefno']."' AND stsNo = '".$arr['stsNo_'.$i]."' AND stsApplyDate >= '$cancelDate';";
				if($trans){
				...
				if($this->getRecCount($this->execQry($sqlCountCancelled)) == 1){
					$sqlUpdateSTSHdr = "UPDATE tblstshdr SET stsStat = 'C', cancelDate = '".date('m/d/Y',strtotime($cancelDate))."', cancelledBy = '".$_SESSION['sts-userId']."', cancelId = '".$lastId."' WHERE stsRefNo = '".$arr['hdnRefno']."'";
					if($trans){
						$trans = $this->execQry($sqlUpdateSTSHdr);	
					}
				}
