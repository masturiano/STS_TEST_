...
class allowanceObj extends commonObj {
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsAmt, stsRemarks, 
			...
			stsType, contractTag, contactPerson, contactPersonPos, origStr) 
			VALUES 
			('$tempRefNo', '{$arr['hdnSuppCode']}', '13', '0', '0', 
			'{$arr['txtSTSAmount']}', '{$arr['txtRemarks']}', '{$arr['cmbPayType']}', NULL,
			'{$arr['txtNoApplications']}', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-grpCode']."', 'O', '4', 'Y', '{$arr['txtRep']}', '{$arr['txtRepPos']}','".$_SESSION['sts-strCode']."')";
	...
	function getRegSTSInfoAssoc($refNo){
		$sql = "SELECT     tblStsHdr.stsRefno, tblStsHdr.suppCode, tblStsHdr.stsAmt, tblStsHdr.stsRemarks, tblStsHdr.stsPaymentMode, tblStsHdr.nbrApplication, tblStsHdr.applyDate, tblStsHdr.contactPerson, tblStsHdr.contactPersonPos, sql_mmpgtlib.dbo.APSUPP.ASNAME as suppName
...
FROM         pg_pf.dbo.tblBranches INNER JOIN
	...
	function getFilteredBranches($compCode,$refNo){
		...
		if((int)$_SESSION['sts-strCode']!=901){
			...
			$sql = "SELECT      CAST(pg_pf.dbo.tblBranches.strCode AS nvarchar) + ' - ' + pg_pf.dbo.tblBranches.brnDesc AS brnDesc, pg_pf.dbo.tblBranches.strCode, pg_pf.dbo.tblBranches.compCode,
                          (SELECT     stsAmt
                            FROM          tblstsdtl
                            WHERE      compCode = pg_pf.dbo.tblBranches.compCode AND strCode = pg_pf.dbo.tblBranches.strCode AND stsRefNo = $refNo) AS stsAmt
FROM         pg_pf.dbo.tblBranches WHERE compCode is not null $filter $filter2 order by strCode";	
		//}else{
...
FROM         tblStsDtl INNER JOIN
                      pg_pf.dbo.tblBranches ON tblStsDtl.compCode = pg_pf.dbo.tblBranches.compCode AND tblStsDtl.strCode = pg_pf.dbo.tblBranches.strCode WHERE tblStsDtl.stsRefno = '$refNo' AND tblStsDtl.strCode not in (SELECT     strCode
                            FROM          tblStsEnhanceDtl
                            WHERE      (stsRefno = $refNo))";
		return $this->getArrRes($this->execQry($sql));
	}
	function getEnhancerBranchesWithBrand($refNo,$brandCode,$category){
		$sql = "SELECT     tblStsDtl.strCode, pg_pf.dbo.tblBranches.brnDesc, tblStsDtl.stsRefno, tblStsEnhanceDtl.enhanceType
FROM         tblStsDtl INNER JOIN
                      pg_pf.dbo.tblBranches ON tblStsDtl.compCode = pg_pf.dbo.tblBranches.compCode AND tblStsDtl.strCode = pg_pf.dbo.tblBranches.strCode left outer  JOIN
...
WHERE (tblStsDtl.stsRefno = $refNo) AND (tblStsEnhanceDtl.brandCode = $brandCode) AND (tblStsEnhanceDtl.category = $category) ";
	...
	function getAllCategory(){
		$sql = "SELECT * FROM tblCategory WHERE stat = 'A'";
		return $this->getArrRes($this->execQry($sql));
	}
