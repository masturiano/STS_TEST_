...
FROM         tblStsHdr INNER JOIN
					  ...
					  WHERE $dt BETWEEN '$dtStart' AND '$dtEnd' AND tblStsHdr.suppCode = '$suppCode' $filter $stsType $grpVar ";	
		return $this->getArrRes($this->execQry($sql));
	}
...
WHERE $dt  BETWEEN '$dtStart' AND '$dtEnd' $stsType $filter1 $filter2 $grpVar ORDER BY pg_pf.dbo.tblStsHdr.suppCode ";	
		return $this->getArrRes($this->execQry($sql));
	}
	function cancelledSTSSummary($dtFrom,$dtTo,$trans){
		...
		$sql = "SELECT     tblCancelledSTS.stsRefno, tblStsHdr.stsRemarks, pg_pf.dbo.tblSuppliers.suppName AS suppName, tblStsHdr.stsAmt, tblStsHdr.stsStartNo, 
                      tblStsHdr.stsEndNo, tblStsHdr.cancelDate, tblCancelType.cancelDesc, tblUsers.fullname, SUM(tblCancelledSTS.uploadedAmt) AS uploadeAmt, 
                      SUM(tblCancelledSTS.queueAmt) AS queueAmt, tblCancelledSTS.cancelDate AS effectivityDate
FROM         tblCancelledSTS INNER JOIN
                      tblStsHdr ON tblCancelledSTS.stsRefno = tblStsHdr.stsRefno INNER JOIN
                      pg_pf.dbo.tblSuppliers ON tblCancelledSTS.suppCode = pg_pf.dbo.tblSuppliers.suppCode INNER JOIN
                      tblUsers ON tblCancelledSTS.cancelledBy = tblUsers.userId INNER JOIN
                      tblCancelType ON tblStsHdr.cancelId = tblCancelType.cancelId
					  WHERE tblStsHdr.cancelDate BETWEEN '$dtFrom' AND '$dtTo' 
					  $stsType 
GROUP BY tblCancelledSTS.stsRefno, tblStsHdr.stsRemarks, pg_pf.dbo.tblSuppliers.suppName, tblStsHdr.stsAmt, tblStsHdr.stsStartNo, tblStsHdr.stsEndNo, 
                      tblStsHdr.cancelDate, tblCancelType.cancelDesc, tblUsers.fullname, tblCancelledSTS.uploadedAmt, tblCancelledSTS.queueAmt, 
                      tblCancelledSTS.cancelDate";
		return $this->getArrRes($this->execQry($sql));
	}
	/*
...
FROM         tblStsDtl INNER JOIN
	...
	function getParDetail2($trans,$dtStart,$dtEnd,$stat,$suppCode,$grp){
		...
		FROM         tblStsDtl INNER JOIN
                      ...
                      tblGroup ON tblStsHdr.grpCode = tblGroup.grpCode WHERE $dt BETWEEN '$dtStart' AND '$dtEnd' 
...
WHERE     ($tbl.uploadDate BETWEEN '$dtStart' AND '$dtEnd') AND (tblStsHierarchy.levelCode = '3') $colQ";
		return $this->getArrRes($this->execQry($sql));
	}
	function findGroup(){
