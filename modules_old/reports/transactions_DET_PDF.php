		...
		$reportsObj = new reportsObj();
			...
			foreach ($arrSupp as $valSup) {
				...
				foreach ($arrH as $valH) {
					...
					$this->Cell(25,5,date('m/d/Y',strtotime($valH['applyDate'])),0,0,'L');
					...
					$arrDet = $reportsObj->getParDetail($valH['stsRefno']);
					$this->SetFont('Arial', 'B', '8');
					$this->Cell(20,5,'',0,0,'L');
					if($valH['stsType']=='5'){
						$this->Cell(50,5,'DA NO',0,0,'L');
					}else{
						$this->Cell(50,5,'STS NO',0,0,'L');
					}
					...
					foreach ($arrDet as $valDet) {
						...
						$this->Cell(60,5,$valDet['brnShortDesc'],0,0,'L');
