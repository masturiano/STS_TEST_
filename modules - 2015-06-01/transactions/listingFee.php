...
switch($_GET['action']){	
	case 'Load':
		...
		/*if($_GET['_search']=='true'){
			$arrSTS= $listObj->searchDispSTS($sidx,$sord,$start,$limit,$_GET['searchField'],$_GET['searchString']);
		}else{	
			$arrSTS = $listObj->getPaginatedDispSTS($sidx,$sord,$start,$limit);
		}*/
		if(isset($_GET['refNo'])){
			$arrSTS = $listObj->getPaginatedDispSTSSearch($sidx,$sord,$start,$limit,$_GET['refNo']);
		}else{
			$arrSTS = $listObj->getPaginatedDispSTS($sidx,$sord,$start,$limit);
		}
		$i = 0;
	...
	case 'searchSupplier':
		$arrResult = array();
		$arrSupp = $listObj->findSupplier($_GET['term']);
	...
	case 'cmbClass':
		$listObj->DropDownMenu($listObj->makeArr($listObj->findClass($_GET['dept']),'stsCls','hierarchyDesc',''),'cmbClass','','class="selectBox" onchange="loadSubClass(this.value);"');
	exit();
	break;

	case 'cmbSubClass':
		$listObj->DropDownMenu($listObj->makeArr($listObj->findSubClass($_GET['dept'],$_GET['class']),'stsSubCls','hierarchyDesc',''),'cmbSubClass','','class="selectBox" ');
	exit();
	break;
	
	case 'cmbStore':
		$listObj->DropDownMenu($listObj->makeArr($listObj->findStore($_GET['compCode']),'brnCode','brnDesc',''),'cmbStore','','class="selectBox" ');
	exit();
	break;
	case 'saveHdr':
		if($listObj->saveHeader($_GET)){
			$lastQuery = $listObj->getLastSTSInserted();
			...
			echo "LoadData('listingFee.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+{$lastQuery['stsRefNo']});";
	...
	case 'getInfo':
		$val = $listObj->getRegSTSInfoAssoc($_GET['refNo']);
		$stsAmt = $listObj->getAmtPerStr($_GET['refNo']);
		echo "$('#txtRefNo').html('{$val['stsRefno']}');\n";
		
		echo "$('#txtAmtPerStr').val('{$stsAmt}');\n";
	...
	case 'loadEnhancerDetails':
		$arr = $listObj->getEnhancerDetails($_GET['refNo']);
		echo "
	...
	break;
	case 'LoadSTSDetails':
		$arr = $listObj->getRegSTSDetails($_GET['refNo']);
	...
	case 'DeleteSTS':
		if($listObj->DeleteSTS($_GET['refNo'])) {
	...
	case 'ReleaseSTS':
		if($_SESSION['sts-userLevel']==1){
			$count = $listObj->hasSTSDetail($_GET['refNo']);
			if((int)$count>0){
				if($listObj->releaseSTS($_GET['refNo'])){
					echo "$('#dialogAlert').dialog('close');\n";
	...
	case "printContract":
			if ($listObj->PrintContract((int)$_GET['refNo']))	{
	...
	case 'getSTSToCancelInfo':
		$Up = $listObj->calculateUploadedAmtSum($_GET['refNo']);
		$Q = $listObj->calculateQueuedAmtSum($_GET['refNo']);
		if($Q['stsApplyAmt'] == 0){
	...
	case 'getCancellationDate':
		$listObj->DropDownMenuD($listObj->makeArr($listObj->getCancelDates($_GET['refNo']),'stsApplyDate','stsApplyDate',''),'cmbCancelDate','','class="selectBox" ');
	exit();
	break;
	
	case 'CancelSTS':
		if($listObj->cancelSTS($_GET['refNo'],$_GET['txtReason'],$_GET['cmbCancelDate'])){
...
$arrAmt = array('2600.00'=>"2600.00",'3000.00'=>"3000.00");
?>
...
$(function(){
	...
	$("#stsTable").jqGrid({ 
		url:'listingFee.php?action=Load', 
		datatype: "json", 
	...
	$("#txtSupp").autocomplete({
		source: "listingFee.php?action=searchSupplier",
		minLength: 1,
...
function AddSTSExt(){
	$("#txtRemarks").val("Type in the products descriptions here...");
			...
			'Save': function() {
				if(validateEnhacerDtl()){
					$.ajax({
						...
						success: function(Data){
							LoadData2('listingFee.php','enhancerData','enhancerDetailList','action=loadEnhancerDetails&refNo='+refNo);
							ProcessData('','Close');
							eval(Data);
						}				
					});
...
function searchGrid(){
	...
	$("#stsTable").jqGrid('setGridParam',{url:"listingFee.php?action=Load&refNo="+refNo,page:1}).trigger("reloadGrid");
}
...
function EditSTS(){
	...
	setTimeout("LoadData('listingFee.php','Data','STSDetailsList','action=LoadSTSDetails&refNo="+refNo+"');",2900);
	//LoadData2('listingFee.php','enhancerData','enhancerDetailList','action=loadEnhancerDetails&refNo='+refNo);
	$("#dialogAdd").dialog("destroy");
	$("#dialogAdd").dialog({
...
function AddSTSDtl(compCode) {
	var refNo = $('#hdnRefNo').val();
	var defaultAmt = $('#txtAmtPerStr').val();
	...
	$.ajax({
		url: 'listingDtl.php',
		type: "GET",
		data: "act=Details&refNo="+refNo+'&compCode='+compCode+'&defaultAmt='+defaultAmt+'&vatTag='+vatTag,
		beforeSend: function() {
			ProcessData('Getting list of Branches Please wait...','Open');
		},
		success: function(Data){
		...
		buttons: {
			'Save': function() {
				$.ajax({
					...
					success: function(Data){
						LoadData('listingFee.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+refNo);
						ProcessData('','Close');
						eval(Data);
					}				
				});										
...
function updateHdr(){
	...
	if(validateInputs()){
		$.ajax({
			...
			success: function(msg){
				LoadData('listingFee.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+stsRefNo);
				eval(msg);
			}				
		});
	}else{
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">Listing Fee</h2>
<br />
	...
	<div style=" visibility:hidden;  overflow:hidden;">
        <div id='dialogAdd' title='Add'>
            ...
            <table width="100%" border="0" cellspacing="2" cellpadding="0" align="center">
                ...
                <tr>
                    <td  class="hd">Amount Per Store: </td>
                    <td><? $listObj->DropDownMenu($arrAmt,'txtAmtPerStr','','class="selectBox"'); ?>
                   </td>
                     <td class="hd">Payment Mode: </td> 
                    <td><? $listObj->DropDownMenu($arrMode,'cmbPayType','','class="selectBox2" onchange="enableVat(this.value);"'); ?> W/ VAT <input type="checkbox" id="ckVat" name="ckVat"/></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="3"><input type="text" name="txtRemarks" id="txtRemarks" class="textBoxLong" onclick="clearTextRemarks();"/></td>
                </tr>
