	...
	$worksheet->write(2,10,"EFFECTIVITY DATE",$headerFormat);
		...
		$arrCancelled = $reportsObj->cancelledSTSSummary($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbTran']);
			...
			foreach ($arrCancelled as $val) {
				...
				$ctr++;
				$worksheet->write($ctr,2,"STS No.",$row3);
				$worksheet->write($ctr,3,"Store Amount",$row3);
				$worksheet->write($ctr,4,"Uploaded Amount",$row3);
				$worksheet->write($ctr,5,"Onqueue Amount",$row3);
				$worksheet->write($ctr,6,"Group",$row3);
				$worksheet->write($ctr,7,"Hierarchy",$row3);
				...
				foreach ($arrCancelledD as $valD) {
					...
					$col = ($col==0) ? 1:0;
					$ctr++;	
					$worksheet->write($ctr,2,"STS".$valD['stsNo']."-".$valD['stsSeq'],$row);
