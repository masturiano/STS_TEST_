...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,"Expiring Contract for the Month of ".date('m-Y',strtotime($_GET['dtMonthYr'])). " ".$strFilt ,$headerFormat);
	for($i=1;$i<10;$i++) {
	...
	$worksheet->write(2,7,"ENTERED BY",$headerFormat);
		...
		$arrExp = $reportsObj->expiredContractsSumm(date('m/d/Y',strtotime($_GET['dtMonthYr'])),$_GET['cmbTran']);
			...
			foreach ($arrExp as $val) {
				...
				$arrDtl = $reportsObj->expiredContractDtl($val['stsRefno'],$_GET['cmbTran'],$_GET['cmbStr']);
