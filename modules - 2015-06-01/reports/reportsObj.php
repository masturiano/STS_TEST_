...
class reportsObj extends commonObj {
	...
	function searchBranch($strCode){
		$sql = "SELECT strCode, brnDesc FROM pg_pf..tblBranches WHERE strCode = '$strCode'";	
		return $this->getSqlAssoc($this->execQry($sql));
	}
...
FROM         tblStsApply INNER JOIN
	...
	function transSummary($trans,$dtStart,$dtEnd,$stat,$suppCode,$grp,$strCode,$payMode){
		if($stat != '0')
		...
		if((int)$payMode <> 0){
			$pay = "AND tblStsHdr.stsPaymentMode = '$payMode'";
		}else{
			$pay = "";
		}
		if($strCode <> 0){
			$str = "AND tblStsHdr.strCode = '$strCode'";	
		}else{
			$str = "";
		}
...
FROM         tblStsHdr INNER JOIN
					  ...
					  WHERE $dt BETWEEN '$dtStart' AND '$dtEnd' AND tblStsHdr.suppCode = '$suppCode' AND tblStsHdr.stsStat = '$stat' $filter $stsType $grpVar $pay AND tblStsHdr.stsRefno in (SELECT distinct stsRefno FROM tblStsDtl)";	
		return $this->getArrRes($this->execQry($sql));
	}
	function transSummarySupp($trans,$dtStart,$dtEnd,$stat,$supp,$grp,$strCode,$payMode){
		if($stat != '0')
		...
		if($strCode <> 0){
			$str = "WHERE tblStsDtl.strCode = '$strCode'";	
		}else{
			$str = "";
		}
...
WHERE $dt  BETWEEN '$dtStart' AND '$dtEnd' $stsType $filter1 $filter2 $grpVar $pay  AND tblStsHdr.stsRefno in (SELECT distinct stsRefno FROM tblStsDtl $str) ORDER BY pg_pf.dbo.tblStsHdr.suppCode ";	
		return $this->getArrRes($this->execQry($sql));
	}
...
FROM         tblStsDtl INNER JOIN
	...
	//Create
	function getParDetail2($trans,$dtStart,$dtEnd,$stat,$suppCode,$grp,$strCode,$payMode){
		//if($trans != '0'){
		...
		if($strCode <> 0){
			$str = "AND tblStsDtl.strCode = '$strCode'";	
		}else{
			$str = "";
		}
		...
		isnull(tblStsDtl.eventNo,tblStsHdr.eventNo) as eventNo, CASE tblStsDtl.eventType
				WHEN 'R' THEN 'Regular'
				WHEN 'A' THEN 'Add Promo'
				ELSE tblStsDtl.eventType
		END AS eventType
		FROM         tblStsDtl left JOIN
                      pg_pf.dbo.tblBranches ON tblStsDtl.strCode = pg_pf.dbo.tblBranches.strCode AND tblStsDtl.compCode = pg_pf.dbo.tblBranches.compCode left JOIN
		...
		AND tblStsHdr.stsStat = '$stat' AND tblStsHdr.stsRefno in (SELECT distinct stsRefno FROM tblStsDtl)
	  	$grpVar
		$stsType $pay $str";
...
FROM    tblDisplaySpecs 
	...
	function getParDetail3($trans,$dtStart,$dtEnd,$stat,$suppCode,$grp,$strCode,$payMode){
		if($trans==0){
			$trans_q = "";	
		}else{
			$trans_q = "AND tblStsHdr.stsType = '$trans'";	
		...
		$stsType";	*/	
		 $sql = "SELECT     tblStsDtl.stsRefno, tblStsHdr.suppCode + ' - ' + sql_mmpgtlib.dbo.APSUPP.ASNAME AS supplier, tblStsDtl.stsNo, 
                      ...
                      tblStsHdr.contractNo, tblStsDtl.compCode, tblStsHdr.nbrApplication, tblDisplaySpecs.displaySpecsDesc, tblStsHdr.stsRemarks, 
...
FROM         tblStsApply INNER JOIN pg_pf.dbo.tblBranches ON tblStsApply.compCode = pg_pf.dbo.tblBranches.compCode AND tblStsApply.strCode = pg_pf.dbo.tblBranches.strCode INNER JOIN sql_mmpgtlib.dbo.APADDR ON tblStsApply.suppCode = sql_mmpgtlib.dbo.APADDR.AANUM WHERE tblStsApply.stsApplyDate BETWEEN '$dtFrom' AND '$dtTo' $stat $stsType";
	...
	function cancelledSTSDetail($dtFrom,$dtTo,$trans,$pMode,$strCode){
		if($trans != '0'){
			$stsType = "AND tblStsHdr.stsType = $trans";	
		}
		if($pMode != '0'){
...
FROM         tblCancelledSTS INNER JOIN
                      pg_pf.dbo.tblBranches ON tblCancelledSTS.strCode = pg_pf.dbo.tblBranches.strCode left JOIN
		...
		*/
		if($strCode <> 0){
			$str = "AND tblCancelledSTS.strCode = '$strCode'";	
		}else{
			$str = "";
		}

		$sql = "SELECT     tblStsHdr.stsRefno, tblStsHdr.stsRemarks, tblCancelledSTS.stsNo, tblStsDtl.stsAmt, tblCancelledSTS.stsSeq, tblCancelledSTS.uploadedAmt, 
                      ...
                      sql_mmpgtlib.dbo.APSUPP.ASNAME AS suppName, tblUsers.fullName, tblCancelType.cancelDesc,
...
WHERE     month(CASE WHEN ststype = '5' THEN endDate ELSE DATEADD(month, nbrApplication - 1, applyDate) END) = month('".$monthYear."') AND 
	...
	function expiredContractDtl($stsRefNo,$trans,$strCode){
		if($trans != '0'){
			$stsType = "AND tblStsHdr.stsType = $trans";	
		}
		if($strCode <> 0){
...
WHERE     (tblStsDtl.stsNo IS NOT NULL) and tblStsDtl.stsRefno = '$stsRefNo' $stsType $str";	
		return $this->getArrRes($this->execQry($sql));
	}
