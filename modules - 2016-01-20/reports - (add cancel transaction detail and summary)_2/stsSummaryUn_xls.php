...
################### INCLUDE FILE #################
	...
	$worksheet = &$workbook->addWorksheet("unapproved_STS_summary");
	...
	$worksheet->write(0,0,"Unapproved STS Summary $type From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])). " Group: ".$grpName,$headerFormat);
	for($i=1;$i<=9;$i++) {
	...
	$worksheet->write(2,1,"No. of App",$headerFormat);
	...
	$worksheet->write(2,6,"Entered By",$headerFormat);
	$worksheet->write(2,7,'Mode of Payment',$headerFormat);
	$worksheet->write(2,8,"Transaction Type",$headerFormat);
	$worksheet->write(2,9,"Remarks",$headerFormat);
	
	$ctrRow = 3;
	$totFund = $totUpload = $totQueue = 0;
	$ctr = 3 ;
	$arrTran = $reportsObj->stsSummaryUnapproved($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_SESSION['sts-grpCode'],$_GET['cmbSupp']);
		...
		foreach ($arrTran as $valD) {
			...
			$worksheet->write($ctr,1,$valD['nbrApplication'],$row2);
			...
			$worksheet->write($ctr,5,date('m/d/Y',strtotime($valD['dateEntered'])),$row2);
			$worksheet->write($ctr,6,$valD['userName'],$row2);
			$worksheet->write($ctr,7,$valD['payMode'],$row2);
			$worksheet->write($ctr,8,substr($valD['dept'],0,4)."-".substr($valD['cls'],0,4)."-".substr($valD['subCls'],0,4),$row);
			$worksheet->write($ctr,9,substr($valD['stsRemarks'],0,45),$row);
