...
switch($_GET['action']){	
	...
	case 'saveHdr':
		if($daObj->saveHeader($_GET)){
			...
			//mike
			//LoadData2('rentableMaintenanceFree.php','enhancerData','enhancerDetailList','action=loadEnhancerDetails&refNo='+refNo);
			echo "LoadData2('rentableMaintenanceFree.php','enhancerData','enhancerDetailList','action=loadEnhancerDetails&refNo='+'{$lastQuery['stsRefNo']}');";
...
$(function(){
	...
	$("#stsTable").jqGrid({ 
		url:'rentableMaintenanceFree.php?action=Load', 
		datatype: "json", 
	...
	$("#txtSupp").autocomplete({
		source: "rentableMaintenanceFree.php?action=searchSupplier",
		minLength: 1,
...
function cancelSTS2(){
	var refNo = $('#hdnRefNo').val();
	$.ajax({
		url: 'rentableMaintenanceFree.php',
		type: "GET",
...
function saveHdr(){ 
	...
	if(validateInputs() && valDateStartEnd(dateStart,dateEnd,'txtApDate','txtEndDate')){
		$.ajax({
			url: "rentableMaintenanceFree.php",
			type: "GET",
			traditional: true,
...
function searchGrid(){
	...
	$("#stsTable").jqGrid('setGridParam',{url:"rentableMaintenanceFree.php?action=Load&refNo="+refNo,page:1}).trigger("reloadGrid");
}
...
function EditSTS(){
	...
	//LoadData('rentableMaintenanceFree.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+refNo);
	LoadData2('rentableMaintenanceFree.php','enhancerData','enhancerDetailList','action=loadEnhancerDetails&refNo='+refNo);
	$("#dialogAdd").dialog("destroy");
	$("#dialogAdd").dialog({
		title: "EDIT FREE RENTABLES",
		height: 470,
		width: 750,
		modal: true,
		closeOnEscape: false,
...
function addRentables() {
	...
	$.ajax({
		url: 'rentableFocDtl2.php',
		type: "GET",
		data: "act=getRentables&refNo="+refNo+'&startDate='+$("#txtApDate").val()+'&endDate='+$("#txtEndDate").val(),
		beforeSend: function() {
			ProcessData('Getting list of Branches Please wait...','Open');
		},
		success: function(Data){
...
function updateHdr(){
	...
	if(validateInputs() && valDateStartEnd(dateStart,dateEnd,'txtApDate','txtEndDate')){
		$.ajax({
			...
			success: function(msg){
				LoadData('rentableMaintenanceFree.php','Data','STSDetailsList','action=LoadSTSDetails&refNo='+stsRefNo);
				eval(msg);
			}				
		});
	}else{
...
function loadRentables(id,refNo){
		...
		$.ajax({
			url: "rentablerentableMaintenanceFree.php",
			type: "GET",
			traditional: true,
			data: 'action=getRentables&displaySpecs='+dispSpecs+'&strCode='+strCode+'&refNo='+refNo,
			success: function(msg){
				...
				buttons: {
					'Save': function() {
						$.ajax({
							url: 'rentablerentableMaintenanceFree.php',
							type: "POST",
							data: $("#frmRentable").serialize()+'&act=saveRentables&displaySpecs='+dispSpecs+'&strCode='+strCode+'&refNo='+refNo+'&id='+id,
							success: function(Data){
								ProcessData('','Close');
								eval(Data);
							},
...
<h2 class="ui-widget-header ui-corner-all" style="padding:5px;">FREE OF CHARGE RENTABLES</h2>
<br />
...
<div id="stsPager"></div> <br /> 
	...
	<div style=" visibility:hidden;  overflow:hidden;">
        <div id='dialogAdd' title='Add'>
            ...
            <h4 class="ui-widget-header">Free Rentables Details</h4>
