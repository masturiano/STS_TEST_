...
################### INCLUDE FILE #################
	...
	$filename = "sts_by_branch.xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("STS by Branch");
	...
	$worksheet->write(0,0,"$status $type From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo']))." By Branch",$headerFormat);
	for($i=1;$i<=10;$i++) {
	...
	$worksheet->write(2,10,"MMS REF",$headerFormat);
		...
		$ctr = 3 ;
		$arrTran = $reportsObj->getStsBranch($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$_GET['cmbTran']);
			...
			foreach ($arrTran as $val) {
				...
				$col = ($col==0) ? 1:0;
				$arrH = $reportsObj->getReleasedSTSAP($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$val['strCode'],$_GET['cmbTran']);
				$ctr++;
				$worksheet->write($ctr,0,$val['brnShortDesc'],$row2);
