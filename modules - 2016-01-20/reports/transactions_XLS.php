...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,"STS Transactions Summarized Report $status $type From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])). " ".$strFilt. " ".$payFilt ,$headerFormat);
	for($i=1;$i<=10;$i++) {
	...
	$worksheet->write(2,1,"EVENT NO",$headerFormat);
	$worksheet->write(2,2,"Amount",$headerFormat);
	$worksheet->write(2,3,"Date Ent.",$headerFormat);
	$worksheet->write(2,4,"App. Start Date-No.",$headerFormat);
	$worksheet->write(2,5,"Mode of Payment",$headerFormat);
	$worksheet->write(2,6,"Contract No.",$headerFormat);
	$worksheet->write(2,7,"Approved Date",$headerFormat);
	$worksheet->write(2,8,"Approved By",$headerFormat);
	$worksheet->write(2,9,"Hierarchy",$headerFormat);
	$worksheet->write(2,10,"Remarks",$headerFormat);
		...
		$arrTran = $reportsObj->transSummarySupp($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$_GET['cmbSupp'],$_GET['cmbGroup'],$_GET['cmbStore'],$_GET['cmbPayMode']);
			...
			foreach ($arrTran as $val) {
				...
				$col = ($col==0) ? 1:0;
				$arrH = $reportsObj->transSummary($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$val['suppCode'],$_GET['cmbGroup'],$_GET['cmbStore'],$_GET['cmbPayMode']);
				$ctr++;
				$worksheet->write($ctr,0,$val['suppName'],$headerFormat2);
				foreach($arrH as $valD){
					$ctr++;
					$worksheet->write($ctr,0,$valD['stsRefno'],$row2);
					$worksheet->write($ctr,1,$valD['eventNo'],$row2);
					$worksheet->write($ctr,2,number_format($valD['stsAmt'],2),$row2);
					$worksheet->write($ctr,3,date('m/d/Y',strtotime($valD['dateEntered'])),$row2);
					$worksheet->write($ctr,4,($valD['applyDate'] !="") ? date('m/d/Y',strtotime($valD['applyDate']))."-".$valD['nbrApplication']:"",$row);
					$worksheet->write($ctr,5,$valD['payMode'],$row2);
					$worksheet->write($ctr,6,$valD['contractNo'],$row2);
					$worksheet->write($ctr,7,date('m/d/Y',strtotime($valD['dateApproved'])),$row2);
					$worksheet->write($ctr,8,$valD['fullName'],$row2);
					$worksheet->write($ctr,9,$valD['dept']."-".$valD['cls']."-".$valD['subCls'],$row);
					$worksheet->write($ctr,10,$valD['stsRemarks'],$row);
