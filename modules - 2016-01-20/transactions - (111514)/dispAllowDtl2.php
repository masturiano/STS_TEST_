 ...
 function CopyTxt(objSource,objClip) {
	...
	function ToggleBranch(id) {
		...
		if (chckBox.attr('checked')) {
			...
			$('#spand_'+id).removeClass("txtInv");
			$('#spand_'+id).addClass('txtVis2');
			...
			$('#spand_'+id).removeClass("txtVis2");
			$('#spand_'+id).addClass('txtInv');
			$('#txt_'+id).val('');
			$('#txtVatAmt_'+id).val('');
			$('#txtEwtAmt_'+id).val('');
			$('#txtSubAmt_'+id).val('');
	...
	function checkAllDtl() {
		...
		if ($('#chAll').attr('checked')) {
			for(i=0;i<=chldCnt;i++){
				...
				$('#txtDispSpecs_'+i).removeClass("txtInv");
				$('#txtDispSpecs_'+i).addClass('txtVis2');
				$('#spand_'+i).removeClass("txtInv");
			$('#spand_'+i).addClass('txtVis2');
			...
			for(i=0;i<=chldCnt;i++){
				...
				$('#txtEwtAmt_'+i).val('');
				$('#txtSubAmt_'+i).val('');
				$('#spand_'+i).removeClass("txtVis2");
			$('#spand_'+i).addClass('txtInv');
          ...
          <tr>
            ...
            <tr>
              ...
              <td height="25"><?=$val['brnDesc'];?></td>
               <td align="center"><? $allowanceObj->DropDownMenu($allowanceObj->makeArr($allowanceObj->getDisplaySpecs(),'displaySpecsId','displaySpecsDesc',''),'txtDispSpecs_'.$i,''. $val['dispSpecs'].'','class="selectBox3 '.$txtInv.'" onchange="enableMenu(this.value,'.$i.')" '); ?> <span class="<?=$txtInv?> link2" id="spand_<?=$i;?>" name="spand_<?=$i;?>" onClick="loadRentables('<?=$i?>','<?=$_POST['refNo']?>');">[...]</span></td>
