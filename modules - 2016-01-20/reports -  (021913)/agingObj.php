...
WHERE     (tblStsHdr.stsStat = 'O') AND (DATEDIFF(day, tblStsHdr.dateEntered, GETDATE()) > 30) $filter AND stsPaymentMode = '$payMode'  ";
	...
	function getAgingStsSumm($type,$payMode){
		...
		$sql = "SELECT     COUNT(m.stsRefno) AS transact, tblGroup.grpDesc, SUM(m.stsAmt) AS amount,
                          (SELECT     SUM(b.stsAmt)
                            FROM          tblStsHdr b
                            WHERE      stsStat = 'O' AND (DATEDIFF(day, dateEntered, GETDATE()) BETWEEN 31 AND 60) AND b.grpCode = m.grpCode) AS over30,
                          (SELECT     SUM(stsAmt)
                            FROM          tblStsHdr a
                            WHERE      stsStat = 'O' AND (DATEDIFF(day, dateEntered, GETDATE()) BETWEEN 61 AND 90) AND a.grpCode = m.grpCode) AS over60,
                          (SELECT     SUM(stsAmt)
                            FROM          tblStsHdr c
                            WHERE      stsStat = 'O' AND (DATEDIFF(day, dateEntered, GETDATE()) > 90) AND c.grpCode = m.grpCode) AS over90
