        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="details">
          ...
          <tr>
            ...
            foreach($arrBranches as $val) {
				$checked = ($val['daSize'] != '') ? 'checked':'';
           		$readonly = ($val['daSize'] != '') ? "class='textBox2 enabletxt' value=".$val['stsAmt']:" readonly class='textBox2'";
				$txtInv =  ($val['daSize'] != '') ? "":"txtInv";
				$switcher_val =  ($val['daSize'] != '') ? "1":"0";
            ...
            <tr>
              ...
              <td align="center"><? $daObj->DropDownMenu($daObj->makeArr($daObj->getDisplaySpecs(),'displaySpecsId','displaySpecsDesc',''),'txtDispSpecs_'.$i,''. $val['dispSpecs'].'','class="selectBox3 '.$txtInv.'"'); ?>   <span class="link <?=$txtInv;?>" style="color:#F60;" id="font_<?=$i;?>" name="font_<?=$i;?>" onClick="loadRentables(<?=$i;?>,'<?=$_POST['refNo']?>');">[...]</span></td>	
              
              
              <td align="center"><input type="text" name="txtNoUnits_<?=$i;?>" id="txtNoUnits_<?=$i;?>" class="textBoxTempSmall <?=$txtInv?>" onKeyPress='return valNumInputDec(event); ' style="text-align:right;"  value="<?=$val['noUnits'];?>"/></td>
