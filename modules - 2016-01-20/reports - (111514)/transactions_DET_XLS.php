...
################### INCLUDE FILE #################
	...
	if((int)$_GET['cmbStore']==0){
		$strFilt = "All Store";
	}else{
		$branch = $reportsObj->searchBranch($_GET['cmbStore']);
		$strFilt = $branch['brnDesc'];
	}
	
	if((int)$_GET['cmbPayMode']==0){
		$payFilt = "All Payment Type";
	}elseif((int)$_GET['cmbPayMode']==1){
		$payFilt = "Invoice Deduction";
	}else{
		$payFilt = "Check / Collection";	
	}
	$worksheet->write(0,0,"STS Transactions Detailed Report $status $type From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])) . " ".$strFilt. " ".$payFilt ,$headerFormat);
	for($i=1;$i<=17;$i++) {
	...
	*/
	...
	$worksheet->write(2,3,"Sts No.",$headerFormat);
	$worksheet->write(2,4,"Event No.",$headerFormat);
	$worksheet->write(2,5,"Branch",$headerFormat);
	$worksheet->write(2,6,"Sts Amt.",$headerFormat);
	$worksheet->write(2,7,"Date Entered",$headerFormat);
	$worksheet->write(2,8,"App. Start Date-No.",$headerFormat);
	$worksheet->write(2,9,"End Date",$headerFormat);
	$worksheet->write(2,10,"Duration",$headerFormat);
	$worksheet->write(2,11,"Mode of Payment",$headerFormat);
	$worksheet->write(2,12,"Contract No",$headerFormat);
	$worksheet->write(2,13,"Date Approved",$headerFormat);
	$worksheet->write(2,14,"Approved By",$headerFormat);
	$worksheet->write(2,15,"Hierarchy",$headerFormat);
	$worksheet->write(2,16,"Group",$headerFormat);
	$worksheet->write(2,17,"Remarks",$headerFormat);
	$worksheet->write(2,18,"GL CODE",$headerFormat);
	//if($_GET['cmbTran'] == '5'){
	...
	//}
		...
		$ctr = 1 ;
		$arrTran = $reportsObj->transSummarySupp($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$_GET['cmbSupp'],$_GET['cmbGroup'],$_GET['cmbStore'],$_GET['cmbPayMode']);
			...
			foreach ($arrTran as $val) {
				...
				/*foreach($arrH as $valD){
					...
					$arrDet = $reportsObj->getParDetail2($_GET['cmbTran'],$_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbStatus'],$val['suppCode'],$_GET['cmbGroup'],$_GET['cmbStore'],$_GET['cmbPayMode']);
					$ctr++;
					...
					*/
					...
					foreach ($arrDet as $valDet) {
						...
						$worksheet->write($ctr,2,$valDet['companyCode'],$row);
						$worksheet->write($ctr,3,$valDet['stsNo'],$row2);
						$worksheet->write($ctr,4,$valDet['eventNo'],$row2);
						$worksheet->write($ctr,5,$valDet['branch'],$row);
						$worksheet->write($ctr,6,number_format($valDet['stsAmt'],2),$row2);
						$worksheet->write($ctr,7,date('m/d/Y',strtotime($valDet['dateEntered'])),$row3);
						//$worksheet->write($ctr,7,date('m/d/Y',strtotime($valDet['applyDate'])),$row3);
						$worksheet->write($ctr,8,($valDet['applyDate'] !="") ? date('m/d/Y',strtotime($valDet['applyDate']))."-".$valDet['nbrApplication']:"",$row);
						$worksheet->write($ctr,9,date('m/d/Y',strtotime($valDet['endDAte'])),$row3);
						$worksheet->write($ctr,10,($valDet['nbrApplication']),$row3);
						$worksheet->write($ctr,11,$valDet['payMode'],$row);
						$worksheet->write($ctr,12,$valDet['contractNo'],$row);
						if($valDet['stsNo'] != ""){
						$worksheet->write($ctr,13,date('m/d/Y',strtotime($valDet['dateApproved'])),$row3);
						}else{
						$worksheet->write($ctr,14,'',$row3);
						}
						$worksheet->write($ctr,15,$valDet['fullName'],$row2);
						$worksheet->write($ctr,16,$valDet['hierarchyDesc'],$row);
						$worksheet->write($ctr,17,$valDet['grpDesc'],$row);
						$worksheet->write($ctr,18,substr($valDet['stsRemarks'],0,45),$row);
						$worksheet->write($ctr,19,substr($valDet['glCode'],0,45),$row);
						//if($_GET['cmbTran'] == '5'){
					...
					$ctr++;
					$grandAmt += $totAmt;
				$worksheet->write($ctr,5,"Sub Total",$headerFormat);
				$worksheet->write($ctr,6,number_format($totAmt,2),$headerFormat);
				}
			if($grandAmt != ""){
			$ctr++;
			$worksheet->write($ctr,5,"Grand Total",$headerFormat);
			$worksheet->write($ctr,6,number_format($grandAmt,2),$headerFormat);
			}
$workbook->close();
?>
