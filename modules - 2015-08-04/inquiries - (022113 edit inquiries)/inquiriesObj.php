...
FROM         tblStsHdr INNER JOIN
	...
	function getSTSHdrContract($no){
		$sql = "SELECT     tblStsDtl.stsRefno, tblStsDtl.stsNo, tblStsHdr.dateApproved, tblStsHdr.applyDate, tblStsHdr.nbrApplication, tblStsHdr.suppCode, pg_pf.dbo.tblBranches.brnShortDesc, 
                      ...
                      tblStsHdr.applyDate) AS endDate, tblStsDaDetail.contractNo, sql_mmpgtlib.dbo.APSUPP.ASNAME AS suppName, tblUsers_1.fullName AS approvedBy, 
                      tblStsHdr.dateEntered, tblStsHdr.cancelDate, tblStsHdr.stsStat, tblStsDtl.strCode, payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END
FROM         tblStsHdr INNER JOIN
                      sql_mmpgtlib.dbo.APSUPP ON tblStsHdr.suppCode = sql_mmpgtlib.dbo.APSUPP.ASNUM INNER JOIN
                      tblStsHierarchy ON tblStsHdr.stsDept = tblStsHierarchy.stsDept INNER JOIN
...
FROM         tblStsApply LEFT OUTER JOIN
	...
	function getSTSDet($no){
		$sql ="SELECT     tblStsDtl.stsRefno, tblStsDtl.stsNo, tblStsHdr.dateApproved, tblStsHdr.applyDate, tblStsHdr.nbrApplication, tblStsHdr.suppCode, pg_pf.dbo.tblBranches.brnShortDesc, 
                      tblStsDtl.stsAmt, tblStsHierarchy.hierarchyDesc, tblStsHdr.stsRemarks, tblStsHdr.stsPaymentMode, tblUsers.fullName, DATEADD(month, tblStsHdr.nbrApplication - 1, 
                      tblStsHdr.applyDate) AS endDate, tblStsDaDetail.contractNo, sql_mmpgtlib.dbo.APSUPP.ASNAME AS suppName, tblUsers_1.fullName AS approvedBy, 
                      tblStsHdr.dateEntered, tblStsHdr.cancelDate, tblStsDtl.dtlStatus, tblStsDtl.strCode, payMode = CASE tblStsHdr.stsPaymentMode WHEN 'C' THEN 'Collection/Check' ELSE 'Invoice Deduction' END
