...
################### INCLUDE FILE #################
	...
	$filename = "cancelled_sts(summarized).xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("Cancelled STS Summarized");
	...
	$worksheet->write(0,0,"Summary of Cancelled $type From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])),$headerFormat);
	for($i=1;$i<11;$i++) {
	...
	$worksheet->write(2,10,"EFFECTIVITY DATE",$headerFormat);
		...
		$arrCancelled = $reportsObj->cancelledSTSSummary($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbTran'],$_GET['cmbStore']);
			...
			foreach ($arrCancelled as $val) {
				...
				$worksheet->write($ctr,7,date('m/d/Y',strtotime($val['cancelDate'])),$row3);
				$worksheet->write($ctr,8,$val['fullname'],$row);
				$worksheet->write($ctr,9,$val['cancelDesc'],$row);
				$worksheet->write($ctr,10,date('m/d/Y',strtotime($val['effectivityDate'])),$row3);
