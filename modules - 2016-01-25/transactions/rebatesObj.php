...
class rebatesObj extends commonObj {
	...
	function getPaginatedRebatesSTS($sidx,$sord,$start,$limit){	
	...
	function getPaginatedRebatesSTSComp($sidx,$sord,$start,$limit,$compCode){	
		if($compCode=='0'){
			$comp = "";	
		}else{
		...
		$sql = "SELECT
			...
			LEFT OUTER JOIN tblsuppliers ON tblstshdr.suppCode = tblsuppliers.suppCode
			LEFT OUTER JOIN tblcompany ON tblstshdr.stsComp = tblcompany.compCode
			WHERE stsType = '2' $comp $filter
			ORDER BY $sidx $sord LIMIT $start , $limit
			";
		return $this->getArrRes($this->execQry($sql));
	}
	
	function searchRebatesSTS($sidx,$sord,$start,$limit,$searchField,$searchString){
		if ($_SESSION['sts-userLevel']!= 1)
			$filter = " AND tblstshdr.grpEntered='{$_SESSION['sts-minCode']}' ";
		...
		$sql = "SELECT
			...
			LEFT OUTER JOIN tblsuppliers ON tblstshdr.suppCode = tblsuppliers.suppCode
			LEFT OUTER JOIN tblcompany ON tblstshdr.stsComp = tblcompany.compCode
			WHERE $searchField = '$searchString' AND stsType = '2' $filter ORDER BY $sidx $sord LIMIT $start , $limit
			";
	...
	function saveHeader($arr){
		...
		$sqlInsert = "INSERT INTO tblStsHdr (stsRefNo, suppCode, stsDept, stsCls, stsSubCls, stsAmt, stsRemarks, 
			...
			'{$arr['txtNoApplications']}', '{$arr['txtApDate']}','".$_SESSION['sts-userId']."', '".$now."', '".$_SESSION['sts-minCode']."', '{$arr['cmbCompCode']}', 'O', '2', '{$arr['cmbSuppCurr']}')";
