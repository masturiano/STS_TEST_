...
################### INCLUDE FILE #################
	...
	$filename = "rentable_status.xls";
	$workbook->send($filename);
	$worksheet = &$workbook->addWorksheet("Cancelled STS");
	$worksheet->setLandscape();
	$worksheet->freezePanes(array(3,0));
	
	$worksheet->setMerge(0, 0, 0, 12);
	if($_GET['cmbAvailTag'] == 'Y'){
		$worksheet->write(0,0,"Available Rentable Display".$strFilt ,$headerFormat);
	}else{
		$worksheet->write(0,0,"Occupied Rentable Display".$strFilt ,$headerFormat);
	}
	for($i=1;$i<13;$i++) {
	...
	$worksheet->write(2,5,"MDSG CATEG.",$headerFormat);
	}
	if($_GET['cmbAvailTag'] == 'N'){
	$worksheet->write(2,6,"APPLICATION START",$headerFormat);
	$worksheet->write(2,7,"APPLICATION END",$headerFormat);
	//$worksheet->write(2,8,"STS REFNO.",$headerFormat);
	...
	//$worksheet->write(2,12,"STATUS",$headerFormat);
	$worksheet->write(2,11,"IMPLEMENT START",$headerFormat);
	$worksheet->write(2,12,"IMPLEMENT END",$headerFormat);
	}
		...
		foreach ($arrRentable as $valD) {
			...
			if($_GET['cmbAvailTag'] == 'N'){
				if($valD['startDate'] == '' || $valD['startDate'] == null){
					...
					$worksheet->write($ctr,6,date('m/d/Y',strtotime($valD['startDate'])),$row1);
				}
				if($valD['endDate'] == '' || $valD['endDate'] == null){
					$worksheet->write($ctr,7,'',$row1);
				}else{
					$worksheet->write($ctr,7,date('m/d/Y',strtotime($valD['endDate'])),$row1);
				}
				//$worksheet->write($ctr,8,$valD['stsRefNo'],$row1);
				if($_GET['cmbAvailTag'] == 'Y' && $valD['availabilityTag'] == 'N'){
					$worksheet->write($ctr,8,'Available',$row1);
				}else{
					//$worksheet->write($ctr,9,$valD['availTag'],$row1);
					$worksheet->write($ctr,8,$valD['stsRefno'],$row1);
				}
				$worksheet->write($ctr,9,$valD['fullName'],$row1);
				$worksheet->write($ctr,10,date('m/d/Y',strtotime($valD['dateCreated'])),$row1);
				//$worksheet->write($ctr,12,$valD['status'],$row1);
