...
################### INCLUDE FILE #################
	...
	$worksheet->write(2,7,"ENTERED BY",$headerFormat);
		...
		$arrExp = $reportsObj->expiredContractsSumm(date('m/d/Y',strtotime($_GET['dtMonthYr'])));
			...
			foreach ($arrExp as $val) {
				...
				$arrDtl = $reportsObj->expiredContractDtl($val['stsRefno']);
				foreach ($arrDtl as $valD) {
					...
					$worksheet->write($ctr,2,"STS ".$sts,$row2);
