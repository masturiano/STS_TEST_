...
################### INCLUDE FILE #################
	...
	$worksheet->write(0,0,"Cancelled $type (Detailed) From ".date('m/d/Y',strtotime($_GET['txtDateFrom']))." to ".date('m/d/Y',strtotime($_GET['txtDateTo'])). " ".$strFilt ,$headerFormat);
	for($i=1;$i<11;$i++) {
	...
	$worksheet->write(2,10,"EFFECTIVITY DATE",$headerFormat);
		...
		$arrCancelledD = $reportsObj->cancelledSTSDetail($_GET['txtDateFrom'],$_GET['txtDateTo'],$_GET['cmbTran'],$_GET['cmbPMode'],$_GET['cmbStore']);
